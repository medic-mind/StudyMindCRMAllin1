...
- **Never auto-mutate** (§2/§3): no auto-charge, auto-delete, or auto-send. AI
  suggests; a human confirms. **One operator-authorised exception:** duplicate
  contacts are **fully auto-merged** with no human step — every cluster sharing
  an email **or** a phone is combined into its oldest record (operator decision
  2026-07, ADR 0047, §37). There is no manual review queue; the
  `CONTACTS_AUTO_MERGE=off` kill-switch is the only human control.
...
3. **No silent data mutation.** Never auto delete, never auto charge, never auto send. AI suggests, humans confirm. **Sole exception (operator-authorised, ADR 0047; widened 2026-07):** duplicate contacts are **fully auto-merged** — the hourly cron (+ a Manager+ "Run now") merges *every* duplicate cluster (any contacts sharing an email or a phone) into its oldest record with no human approval, audited like any merge. This now INCLUDES the phone-only/different-name case (a possible shared family landline, §41.1) — by operator decision there is no manual review queue. The only human control is the `CONTACTS_AUTO_MERGE=off` kill-switch.
...
**Boards and Cards (ADR 0018).** The single pipeline above is generalised into multiple operator-managed **Boards**, each owning its own `PipelineStage` rows (now scoped by `boardId`) and **Card** rows. A Card is backed by a `Contact` (lighter than a Family, which remains the billing unit) and carries an optional `Subject` and any number of coloured `Label`s. Board management (create/rename/reorder/archive), card CRUD and moves are open to every staff role (2026-07 — VA and above can do anything operational; clearing a whole board via `card.clearBoard` stays CEO-only + puzzle-guarded). A card move writes a `card_moved` Interaction on the backing contact plus an audit row. `Family.stageId` is retained for finance/at-risk; `Card.stageId` drives the board display. `/pipeline` redirects to the default board (`/boards/<defaultBoardId>`). Domain: `packages/core/src/board/`; tRPC: `board.*`, `card.*`, `label.*`, `subject.*`, `boardQuickAction.*`; UI: `apps/web/app/(app)/boards/`. The board kanban supports drag-and-drop card moves via @dnd-kit (ADR 0019); the per-card **Move to…** dropdown supports cross-board moves (when the picked target stage lives on another board, the card's `boardId` is updated atomically alongside the stage swap). The whole card body is clickable and opens a detail modal. From a card an agent can record a **call summary** (`card.callSummary.add` → a `call_summary` Interaction on the backing contact) which is announced to `#callsummaries` on Slack — no customer message is sent (simplified in the 2026-07 redesign, ADR 0046, §37).

**Configurable per-card quick actions.** Replaces the legacy single-tick / single-X pair on `Board` (those columns are retained per §19 forward-only but the default board's seed clears them). Each `BoardQuickAction` row defines a chip that appears on every card on the board: it has its own label, colour, target stage (optionally on another board → cross-pipeline routing), and optional comment template. Clicking the chip fires `card.applyQuickAction` which adds the templated comment to the card and moves it to the target stage in one audited operation. CRUD lives in `/boards/[boardId]/settings` (Manager+); firing is Sales Executive+. Default board seeds three buttons (Called once, Called twice, Call completed) plus the user's preferred column layout (New leads · Scheduled 9am-1pm / 1pm-4pm / 4pm-8pm · Called once · Called twice). **"Call completed" and "Never answered" are their OWN boards** (`board_seed_completed_calls`, `board_seed_never_answered`) — their columns were removed from the Sales Pipeline and the quick-action chips route the card CROSS-BOARD there (migrations `20260720130000` / `20260722120000`); fully re-pointable from `/boards/[id]/settings`. An **ANZ Sales Pipeline** board (`board_seed_anz_sales`) is seeded for ANZ-website leads (route a `LeadSource` to it, §37). A **Time Wasters** board (`board_seed_time_waster`) is seeded with a "Time waster" quick-action chip on the Sales Pipeline that routes the card cross-board there; move it back to Sales any time via the per-card Move-to dropdown (migration `20260723220000`). **Tick-circle action (`BoardQuickAction.isCheckbox`, 2026-07):** one quick action per board can render as a Todoist-style **tick-circle** on the card face (by the card name) instead of a chip — the card's primary "done" affordance. The default board seeds **"Call completed"** as the tick-circle (so the circle moves the card cross-board to Completed Calls); operators re-designate it from `/boards/[id]/settings` ("Show as a tick-circle" — one per board, enforced). Circle UI `CardCompleteCircle.tsx`. **The tick-circle action ALSO stays in the chip row** (operator ask 2026-07 — keep both "Call completed" the chip AND the tick-circle, even though they do the same thing); `BoardCard` renders every quick action as a chip and the designated one additionally as the circle. (Full per-action customisation of chip-vs-circle is a later step.)
...
**Missed-calls workspace (`/calls`, all staff; action Sales Executive+).** A queue of inbound calls nobody answered (rang out OR voicemail), including unknown numbers. **A call counts as answered iff Aircall's `answered_at` is set — NOT `duration > 0`** (`isAnswered`, 2026-07): Aircall's `duration` counts the whole call lifetime incl. RING time, so a rung-out call has `duration > 0` while `answered_at` is null; classifying by duration silently recorded those as answered and hid them from this queue (the "missed call didn't show" bug). Both ingest paths now store `payload.answeredAt` (webhook + sync/backfill). Legacy rows (synced before that) reclassify **at read time with no refetch**: a webhook call stores the Aircall event per row, and one that has event rows but no `call.answered` was never picked up (`hasLiveEvent`/`answeredEvent` → `isAnswered`), so historic rung-out **webhook** misses show immediately; the sync also **retroactively enriches** rows missing `answeredAt` (covering the sync/backfill-only case), and the duration heuristic is only the last-resort fallback. The Aircall analytics report (`reports.ts`) shares the same `isAnswered`, so its answered/missed counts never disagree with `/calls`. Voicemails were always caught separately (`isVoicemailPayload`). "Called back" is **derived** — a later outbound call to the same number (format-insensitive match) or to the same linked Contact (any attempt) auto-resolves the miss, so calling back from anywhere clears it with no manual step. The matched "number" is resolved by `callNumberFromPayload` (`payload.rawDigits` for Aircall calls, falling back to `payload.toNumber` for a **manually-logged click-to-call** — the contact-page Call button, and the only record for a Google Voice callback since it has no webhook); without that fallback a manual callback could only clear a miss through a shared contact link, so a callback to an unlinked or differently-linked number silently stayed Outstanding. A manual override (`actioned` / `dismissed` for spam) is the only stored state — `MissedCallReview`, keyed on the Aircall call id. Pure logic (dedupe + number/state) in `packages/core/src/calls/missed-calls.ts` (unit-tested); tRPC `calls.missed.{list,setReview,clearReview}`; UI `MissedCallsWorkspace.tsx` with click-to-call.
...
**Outbound.** Always go through `outbound.ts` so we attach metadata (Interaction id, agent id) to the Trengo message custom fields. This lets us reconcile Trengo events back to our timeline without ambiguity. Agents reply to a conversation from the CRM via `interaction.trengo.reply` (every staff role can send, 2026-07), which resolves the contact's active ticket+channel via the shared `resolveActiveTrengoConversation` helper (`packages/integrations/trengo/src/conversations.ts`) and calls `sendMessage`. The roadmap to a full two-way operational layer (conversation head, real-time SSE, Communication Centre, assign/close/tag sync) is ADR 0020 + `docs/audit/trengo-operational-layer-audit.md`.
...
Anything below threshold, name-ambiguous (two Jane Smiths), or unmatched lands in the "unassigned summaries" tray — **on EVERY ingestion path: the live webhook, the 90-day backfill, AND the recurring/on-demand pull** (they share `processSlackMessage`, which parks any non-noise message it cannot resolve rather than dropping it). This is the fix for "Slack history never showed up": the historic import and the pull used to **silently drop** every message that wasn't an immediate email/phone/AI match — a customer not yet in the CRM, a name-only mention when no AI key is configured, or a first name two customers share — leaving no Interaction, no tray row, and no error. Now those are all recorded (never dropped) — and in full-auto (the default) they are **finalized at the SAME moment** by the shared `finalizeUnresolvedMention` (§37): the archive row is written AND resolved (auto-dismissed) in one operation, so a new un-matchable mention never sits OPEN waiting for a human. The tray is no longer a dead-end either: the recurring **`slack/relink-unassigned`** job (§17.1) re-runs the resolver over every remaining (legacy) open row for free and auto-links the unambiguous ones (draining the backlog + self-healing when a contact/account is created after the mention), so the tray drains automatically with no button (the manual **"Re-run matching now"** and **"Sync from Slack now"** buttons were removed 2026-07 — syncing/matching is automatic-only; the `slackSummary.unassigned.relinkNow` → `slack/relink-now.requested` event remains server-side, just no longer surfaced as a button). The noise filter (acks/emoji/bare links) is the volume control — those are still skipped, never parked. Never auto-attach on ambiguity, never auto-create a contact or account (§3).

**Call-log auto-onboarding + own-brand guard + join-all (ADR 0043; tiers widened 2026-07 — no mention waits for a human).** A Slack message that matches no contact **auto-creates/attaches the customer** on every ingest path incl. the relink cron, by identity tier: **phone** (via the shared call resolver `resolveOrCreateContactForCall`, §10 — fill-blanks on a single match, park on shared lines, never merge §41.1), else **email** (attach to the most recently active contact on that email — ADR 0044 convention — else create), else **full name (2+ words)** — unlocked in call-log channels (`isCallLogChannel`: name contains `summar`/`complaint`) **AND whenever the AI cleared its confidence bar for the customer it named** (operator direction 2026-07: trust the AI's good guess, so a confident mention creates the customer in ANY channel, not only call-log ones — the live + backfill AI paths pass `allowNameOnly:true`, the relink cron passes it when the stored `confidence ≥ 0.5`). Sub-threshold / deterministic-only extractions in generic channels still never create anyone (§11's spam guard, narrowed not dropped); own-brand + noise + full-name guards always apply. The relink cron is **error-isolated per row and scans the whole open backlog in pages** (`RELINK_SCAN_CAP` 2000/tick) — previously one throwing row or 200 unlinkable oldest rows froze everything behind them. Name = the call-log header (text before the number, handles lower-case names) else the first non-brand candidate; `normaliseSlackPhoneToE164` fixes the team's "+44 07818…" paste. **Own-brand names/domains** (live `Company` + `BrandDomainRule` catalogues + seeds, `own-brands.ts`, cached) are filtered from name/email candidates on every path so "Medic Mind" can never hijack a match or become a contact. **"Join all public channels"** (Settings → Integrations → Slack, CEO/SM, audited `slack.channels_joined`) `conversations.join`s every public channel via the `channels:join` scope — the operator-confirmed bulk form of the §12 invite consent; private channels still need `/invite`. The 90-day import runs one Inngest step per page (per-message steps exceeded Inngest's step budget and killed large imports) with delta progress every 10 messages.

**Complaints are logged in the CRM — Slack→complaint auto-ingestion was REMOVED (2026-07, operator decision).** A Slack message is NEVER turned into a `Complaint` row anymore: `maybeRaiseComplaintFromSlack` (`packages/integrations/slack/src/complaints.ts`) is a retained no-op, the "Reprocess complaint mentions" button + `slack/backfill-complaints` are dormant, and the channel-rules complaint helpers (`shouldAutoRaiseComplaint`, `buildComplaintDraft`, `parseStructuredComplaint`) are retained but unused (§19 forward-only; re-enabling needs an ADR). Slack mentions in complaint-flavoured channels are still archived as `slack_summary` interactions (§12 mentions) — they just don't open complaints. Complaints are now logged **in the CRM** on the Complaints hub or a customer's page (§37): staff pick a CRM customer OR type a **manual person** (name + phone, no CRM contact), which ALWAYS posts the complaint to the operator-routed `#complaintcallsummaries` channel via the bot (`complaint_call_summary` topic) and anchors a **Slack thread**; follow-up updates mirror into that thread AND onto the customer's CRM timeline. `Complaint.contactId` is nullable (manual complaints carry `personName/personPhone/personEmail`); full lifecycle — resolve/reopen/dismiss, archive/unarchive, delete/restore, permanently delete.
...
## 13. Asana playbook — REMOVED (2026-07, ADR 0046)

The Asana integration existed **only** to two-way-sync CRM `Task` rows. The
Tasks feature was removed in the 2026-07 redesign (ADR 0046), so the Asana
integration went with it: the `packages/integrations/asana` package, the
`/api/webhooks/asana` receiver, the `AsanaWebhook` table, and the
`asana/event.received` job are all deleted. There is no Asana surface in the
CRM. (This section is retained as historical context; do not reinstate without
an ADR.)
...
**Purchase-alert ride-along (ADR 0048).** Same pattern for **payments**: to pick purchases up **without the Stripe API** (operator security ask — keep Stripe as the processor, no API key in the CRM), `processMessage` routes emails from a configured alert sender (`isPurchaseAlertSender`, `PURCHASE_ALERT_SENDERS`, default `receipts@/notifications@stripe.com`; Stripe sub-addresses match their base) — behind the `stripe.purchase_email_ingest_enabled` flag — to the `webinar/purchase-email.received` event. The apps/web boundary (`_boundary/purchase-email.ts`) AI-extracts `{buyer, product, amount, interval}` (mini-tier `purchase_email` task, format-tolerant), matches/creates the buyer contact, records a `payment` Interaction (`source:'stripe_email'`), and calls `enrollFromPurchase` to enrol confident weekly-class matches (§47). **Record-keeping + webinar service provision only — never charges/refunds/invoices.** Idempotent on the source message id; off until the mailbox is connected + the sender is set.
...
**Enquiry history + IP country + composed phone.** Every enquiry (first + each re-enquiry) is a `lead_enquiry` Interaction surfaced as an **Enquiries** section on the contact page (site, form, subject, products, preferred time, message, IP) via `lead.enquiriesForContact`; a re-enquiry also prepends a one-line "[date] Enquired again: …" summary to the contact's pinned note and upserts the latest subject onto `ContactSubject`, so the contact + new card always reflect the **latest** enquiry. The client **IP** is captured at `/api/leads` (`Lead.ip`); the classify job geo-locates it (best-effort `ipwho.is` → `ipapi.co` → `geojs.io`, all via `safeFetch`, injected at the worker boundary) → `Lead.countryCode`. **Country + phone resolution is one pure, unit-tested function — `resolveLeadPhoneAndCountry` (`packages/core/src/lead/phone-country.ts`)** — with a strict signal priority so a **customer-given country code always wins** (the fix for "≈99.5% of international numbers logged as invalid `+44…`"): **(1) the form country field** — a country NAME/ISO2 (`United Kingdom`/`GB`) OR a **dial code** the CF7 "Country code" field posts (`+44`/`+964`), resolved by `dialCountryFromCode` (`dial-codes.ts`). ⚠️ The missing dial-code reader was a live bug: `findDialCountry` only understood names/ISO2, so a stated `+44`/`+964` was silently dropped, the waterfall fell through to IP-geo of the WordPress host, and the number was mis-composed against the wrong country (`+44→+49…`, `+964→+359…`). **→ (2) the phone's OWN dial code** (explicit `+…`/`00…`, OR a dial code typed without the `+` like "91 98765 43210", recovered by `inferPhoneE164`) **→ (3) a form-forwarded VISITOR IP** (`clientIp`) **→ (4) the AI's inferred country → (5) the TRANSPORT IP (`lead.ip`) — LAST resort.** ⚠️ The transport IP is demoted because a **Contact-Form-7 webhook is POSTed by the WordPress host**, so `lead.ip` is the site server (a UK box), not the enquirer — geolocating it returned "GB" for nearly every overseas lead, which both mangled the number to `+44…` AND (being a "successful" lookup) **pre-empted the phone's own code and the AI**. Now those rank above it. With a country in hand a nationally-typed number ("928 812 118" + Peru) composes to full E.164 via `findDialCountry`/`composePhoneE164` (`packages/core/src/lead/dial-codes.ts` — the **full ISO 3166-1 dial-code set**; a missing country there is a bug, not a gap to tolerate). **A UK-ASSUMED `+44` never blocks the real country:** `normalisePhone` returns `assumedCountry:'GB'` when it optimistically maps a bare `0…`/`7…` national number to `+44` (vs an explicitly-international number → null). The router (`process-lead.ts`) then (a) does NOT read the country back out of a UK-assumed `+44` (this circular confirmation used to lock foreign leads to GB whenever no form country was given and IP geo failed, pre-empting the AI fallback), and (b) recomposes the number with the IP/AI-resolved dial code whenever `assumedCountry==='GB'` and a non-GB country resolves — so a French "06…" from an ANZ/EU IP becomes `+336…`, not `+446…`. A form country always wins; an explicitly-typed international number is never rewritten. (Forward the visitor's IP as a CF7 field so IP geo measures the enquirer, not the WordPress host.) **A typed number always lands on the contact's phone field:** when no country resolves, `inferPhoneE164` recognises a dial code typed without the `+` ("51 928 812 118"; strict 11–15-digit guard so a national number is never misread as international), and if even that fails `asTypedPhoneFallback` stores the digits as typed — visible and manually dialable — instead of burying them in notes. "Phone (as typed)" in notes now only fires for values too short to be a number at all. `Contact.country` is also set from the same resolution. **Fixing the numbers already mangled (forward-only fix ≠ retro fix):** the resolver fix stops NEW leads breaking, but contacts already carrying a wrong `+…` are untouched (the `lead/backfill-countries` job deliberately never overwrites an existing E.164, §3). Two operator tools repair them: (a) **"Fix wrong numbers"** on Settings → Integrations → Lead webhook (`lead.phoneRepair.{preview,apply}` → `scanInvalidNumberRepairs` over the **Invalid number** board's cards → the pure `proposePhoneRepair`, Manager+) re-derives each contact from its ORIGINAL stored enquiry (`Lead.rawPayload`, which still holds the "+44"/"+964" they gave) with the corrected resolver and, after a **preview**, applies only confident customer-stated corrections (audited `contact.updated`, idempotent, skips unique-collisions); (b) anywhere a number renders (board card, contact header, Contacts/at-risk tables) the `PhoneLink` chevron menu offers **Edit number** — an inline `PhoneInput` → `contact.update` (audited) — so staff fix any number in place, incl. ones with no original enquiry to re-derive from (Todoist/call-added).

**Card organisation (site / form / subject / time / board).** Each onboarded card is organised, not just dumped: the detected **Subject** becomes a `Subject` tag on the card (find-or-create, so the board groups by topic); the **site** (`LeadSource.name`) + **form title** are written to `Contact.referralSource` ("Web enquiry · Medic Mind site") and the card/contact notes, and surface as Site/Form/Subject/Board columns in the Leads tray; a **date/time** the enquirer picks on the form (a `[date]`/`[time]`/`datetime` field, a "preferred call time", or a phrase in the message) is parsed (Europe/London → UTC, `londonWallToUtc` in `packages/core/src/lead/`) and set as the card's **Scheduled-call** chip automatically — with an **AI fallback** (`preferredCallTime` on the `lead_classification` mini-task, grounded on the current London date so "Thursday at 3" resolves correctly; shape-validated, deterministic parser always wins) for times the rules can't read. The parser reads split **"Call day" + "Call time"** fields (natural dates like "Friday 24 Jul", weekdays, and time ranges), inferring a future year from the submission date. It also **value-sniffs generic field names**: a call day/time posted under a CF7 auto-name (`text-456`) the synonym list can't recognise is still caught by scanning unassigned fields for an upcoming date/time value (`isUpcomingCallWhenValue` — DOB/age/system-timestamp keys are denylisted, and a far-past date like a `12/05/2008` DOB is rejected via the `now` reference), so it works deterministically without AI — and the back-fill (which re-runs the same normaliser) benefits too. Onboarded cards also carry the **enquiry itself as their note** (`description`), so the board shows a preview at a glance. **Retroactive repair (automatic + on-demand).** A lead is classified within seconds of arriving, so any lead submitted BEFORE the "Call day"/"Call time" parser shipped has a card with a blank scheduled-call — the requested call never reached the board (the live "Shamin Anari" bug: enquired the day before the parser landed, so "Friday 24 Jul 10:00" was missing on the card). Two jobs re-parse the stored payload and fill the card where still blank (both idempotent; year inferred per-lead from `createdAt`, `packages/jobs/src/leads/backfill-call-times.ts`): the recurring **`leads/heal-recent-call-times`** cron (every 30 min, §17.1) auto-fills the pre-parser backlog with NO operator action — bounded to leads submitted in `[now − 90d, LEAD_CALL_TIME_HEAL_BEFORE (default 2026-07-24, the parser go-live))`, so pre-parser cards whose call is still upcoming self-heal but a MODERN card an agent deliberately cleared is never reverted (§3); and the operator button on Settings → Integrations → Lead webhook ("Fill call times on old cards" → `lead.backfillCallTimes` → `leads/backfill-call-times.requested`, Manager+) does the full historic sweep on demand (self-rescheduling). Both share the pure `planCallTimeHeals` planner (unit-tested). The **card face** shows NAME → email → phone → SUBJECT + COMPANY (the contact's first brand tag, a `company` `CardFaceKey`) + **enquiry-type chips** (`enquiryType` key — the lead-classification categories "Tutoring" / "Summer Camp" / "Online Courses" / "UCAT" via `loadContactEnquiryTypes`, latest-first) → scheduled call → the quick-action buttons. **Free-resources routing:** enquiries from download/freebie/guide forms route to a separate **Free Resources** board (`board_seed_free_resources`) instead of the Sales Pipeline — driven by configurable `UrlClassificationRule`s whose category is `Free Resources` (plus a slug heuristic), so ops add more without a developer. The classifier returns `subject` + `destination` (`sales` | `free_resources`) on `LeadClassification`; the board falls back to Sales if Free Resources isn't seeded.
...
| `ai/score-churn-risk` | nightly 03:00 UTC | Score every Family, persist the `ChurnScore`, mirror it onto the Family, and refresh the at-risk derivation (§6.4). High-risk families surface on the at-risk dashboard (no auto-task since the Tasks feature was removed, ADR 0046) |
...
| `compliance/erase-due-records` | daily 04:30 UTC | GDPR right-to-erasure (§21): permanently crypto-shred + anonymise every Contact whose 30-day `erasureScheduledAt` grace window has elapsed (`selectDueErasureContacts` → `eraseContactData`, boundary `_boundary/erase-due-records.ts`). Immediate erasure runs inline via `contact.erase`. Idempotent (skips already-`erasedAt` rows). |
...
| `aircall/sync-calls` | every 10 min | Pull recent Aircall calls (matched + unmatched) via REST so the call mirror stays complete even if a webhook is missed. No-op without `AIRCALL_API_*`. Cursor = newest stored call − **24h** overlap (no cursor table) so a missed/late webhook self-heals within a day; cold-start reaches back `AIRCALL_SYNC_LOOKBACK_DAYS` (default 30) then moves forward. The `aircall/sync-now.requested` event remains server-side, but the manual "Sync from Aircall" button was removed 2026-07 — the call mirror (incl. all missed calls) stays complete automatically via webhooks + this 10-min pull |
...
| `leads/heal-recent-call-times` | every 30 min | Auto-fill `scheduledCallAt` on the cards of leads submitted in `[now − LEAD_CALL_TIME_HEAL_WINDOW_DAYS (default 90), LEAD_CALL_TIME_HEAL_BEFORE (default 2026-07-24))` where it is still blank AND the stored payload carries a parseable "Call day"/"Call time". Fixes the pre-parser backlog (leads classified before the call-time parser shipped) so the requested call surfaces on the board with no operator button. The **go-live cutoff upper bound is deliberate** (mirrors the DD-issues / complaint cutoffs): on a MODERN card `scheduledCallAt: null` can mean an agent *deliberately cleared* the chip, so re-populating it from the old enquiry would be a §3 silent mutation — the heal never touches post-cutoff cards. Cheap + convergent: keyset-paginated ids-only scan of the whole window (never a starving `newest-N`), payloads fetched only for blank cards. Idempotent — only fills a still-blank card. The operator "Fill call times on old cards" button (`leads/backfill-call-times.requested`) remains for a full historic sweep. |
| `contacts/auto-merge-duplicates` | hourly | **Fully** auto-merge duplicate contacts — EVERY cluster sharing an email or a phone is merged into its oldest record with no human step (operator decision 2026-07; `planAutoMerges({ includeAmbiguous: true })`, ADR 0047, §37). Includes the phone-only/different-name case — nothing is parked for review. **Drains the WHOLE backlog per tick**: the boundary loops the service (each pass merges up to the 1000 cap + soft-deletes losers so the next pass's scan finds the next batch) until a pass merges nothing — up to `MAX_PASSES`×1000/tick — so a big historic backlog fully retrospectively merges in one hourly run, not 1000/hour. Audited system merges; skips restricted-access conflicts. No-op with `CONTACTS_AUTO_MERGE=off` (the only human control). |
...
| `slack/sync-messages` | every 15 min | **Pull** recent messages from EVERY channel the bot is a member of (auto-discovered via `conversations.list` → `isMember`, or the `SLACK_WATCHED_CHANNELS` allowlist when set) and process each via the shared backfill processor — so ingestion does NOT depend on the Events-API webhook being configured. No-op without `SLACK_BOT_TOKEN`. Overlap window `SLACK_SYNC_LOOKBACK_MINUTES` (default 120 — overlap costs only DB dedupe checks, never AI re-spend). **Each channel is its OWN Inngest step** (not the whole workspace in one step, which timed out and never committed on a real workspace — the audit's root cause of "not all messages pulled"). The lookback window **paginates to exhaustion** (the old 3-page cap silently dropped older messages on busy channels); a channel whose FIRST page fails is recorded in `failedChannels`, a mid-walk 429 just breaks that page loop (bounded `Retry-After` retries, 6 attempts) and the old-thread scan still runs. Each tick also runs an **old-thread scan** (`SLACK_SYNC_THREAD_SCAN_DAYS`, default 7, up to 10 pages, ALL qualifying roots): thread ROOTS older than the window whose `latest_reply` falls inside it get their new replies walked — `conversations.history` only returns roots, so those replies were structurally invisible to a webhook-less pull. Emits a `CronRun` heartbeat so Settings → Integrations → Slack shows the pull is executing. The `slack/sync-now.requested` event remains server-side, but the manual "Sync from Slack now" button was removed 2026-07 — pulling is automatic-only. The historic bulk pull is still the admin "Import history" backfill — which now also auto-discovers all member channels (it previously enumerated ZERO channels when no allowlist was set, so nothing was ever pulled). Both pull paths also walk each thread's **replies** (`conversations.replies`), since history omits them (§12 Threads/replies). |
| `slack/relink-unassigned` | every 15 min | Re-run the shared resolver (`resolveSlackLinkTarget` — Contact else B2B account, §12) over every open `UnassignedSummary` and auto-link the unambiguous ones (or auto-onboard under the ADR 0043 tiers) as a `slack_summary` (ADR 0034 amendment). **Drains in bounded per-batch Inngest steps** (`relinkParkedRowsBatch`, 100 rows/step, keyset `id > lastId` — NOT the whole backlog in one step, which timed out and never committed, and NOT Prisma cursor+skip, which broke when the cursor row resolved mid-drain); each row is error-isolated so one poison row can't freeze the queue; emits a `CronRun` heartbeat. FREE — reuses the AI extraction already stored on the row + a deterministic email/phone text scan **+ a deterministic name-candidate re-scan of the archived message text** (`extractNameCandidates`, own-brand-filtered → `resolveSlackLinkTargetFromNames`, so rows parked with `name:null` before name extraction existed still self-heal), never calls the AI again. **Fully-automated tray by default (operator direction 2026-07, `slackTrayFullAuto`):** the cron leaves NO row parked for a human — every open row is auto-linked / auto-onboarded OR **auto-dismissed** (`resolveUnlinkedOutcome`): an unrescuable row (`isUnrescuableParkedRow` — no identity + empty/noise text) is always dismissed, and in full-auto mode a substantive-but-nameless row the matcher can't resolve is dismissed too (retained + reversible via `resolvedAt`, never deleted), so the `/inbox/slack-mentions` tray drains to zero on its own. **A row on which the resolver THROWS is force-dismissed in full-auto** (`forceDismissParkedRow`) rather than left parked — the previous behaviour caught the error but never resolved the row, so a class of poison rows (e.g. auto-onboard hitting a constraint) could sit un-drained forever (the "still N unassigned" bug). Now a batch resolves EVERY row it scans (link / dismiss / force-dismiss-on-error); at 2500 rows/run the whole backlog clears in one 15-min tick. Set `SLACK_TRAY_FULL_AUTO=off` to restore the old behaviour (keep a substantive nameless / ambiguous row for a human, §3). **Also retro-stamps** existing contact-linked mentions with their contact's school (`businessAccountId`) so historic mentions surface on the school timeline too (walks the small `BusinessAccountContact` set, fill-blanks `updateMany`, idempotent). Drains the parked backlog and self-heals (a mention parked before its contact/account existed links once it appears). |
| `trengo/reconcile-status` | every 10 min | Re-fetch each Conversation's CURRENT state from Trengo (oldest-checked-first, 120/tick) and re-converge the head — status (open/closed/**spam**), assignee, labels, and the specific **channel** ("business number") — through the same monotonic merger the webhook uses. The safety net so a dropped or unsubscribed lifecycle webhook ("closed on Trengo, still open here") self-heals instead of drifting permanently (golden rule #4). The **`trengo/reconcile-now`** event remains server-side, but the manual "Sync from Trengo" button was removed 2026-07 — convergence is automatic on this cron. No-op without a connected token. A ticket **deleted in Trengo** (a 404 on the pinned endpoint) is converged **out of the active inbox** — its head moves to `archived` (hidden from every active folder, but never hard-deleted — §3 — so it stays recoverable + auditable), so "hidden on Trengo, still showing here" self-heals; an unparsable (non-404) response still leaves the head untouched. Status flips audit `trengo.status_reconciled` (deleted carries `reason:'deleted_in_trengo'`). Each tick is also the **full-mirror engine**: a newest-end discovery scan imports tickets we have never seen (any age) + refreshes recently-active known tickets' messages, and a rotating **deep sweep** walks the whole workspace listing a couple of pages per tick so ALL historic conversations land on customer timelines automatically (unknown senders become Leads + unmatched heads — never auto-created Contacts, §11) |
...
| `webinar/zoom-rotation-reminder` | weekly Mon 08:00 | **Auto-rotate** each class Zoom link older than its rotation interval (default 4 weeks): a fresh open-to-all meeting is created and the old link dies, so the next weekly email carries the new one (ADR 0035 amendment). When a link can't be auto-rotated (per-class `zoomAutoRotate` off, Zoom not connected, or a rotation fails) it **emails the single configured "assigned person"** (`WebinarSettings.rotationReminderEmail`, set in Webinars → Settings) and surfaces the due links in the on-system reminder panel on the Webinars overview — the task-free replacement for the old rotation-reminder Task (ADR 0046). Engine `runZoomRotation` in `apps/web/lib/webinar/zoom-reminder-service.ts`. |
...
| `finance/dd-chase-tick` | hourly | Automated Direct Debit chasing (ADR 0045). **Prelude (best-effort, never blocks chasing):** self-populate + identify the recovery worklist — `linkUnlinkedGcCustomers` → `autoOpenRecoveryCases` → `backfillRecoveryCaseContacts` — so everyone behind on / underpaying is added and identified (name + email from GoCardless, CRM contact auto-created if missing) with zero button presses even when the reconcile→scan chain never fires (self-hosted). **Auto-arm (ON by default, operator decision):** `DdRecoverySettings.autoChaseEnabled` defaults **true**, so each new **un-touched** case is armed automatically — reachable channels on + the single global re-signup link (`autoChaseSetupLinkUrl` / `DD_AUTO_CHASE_LINK`) attached + first message scheduled — via the pure `decideAutoArm` (`packages/core/src/finance/dd-chase.ts`, audited `direct_debit.case_auto_armed`); it never overrides a case a human configured or paused. The ONLY operator input is the re-signup link (you can't send someone to pay with no URL); until it is set the armed case still sends nothing. `DD_AUTO_CHASE=off` pauses the whole thing. This is what makes "automated reminders" actually run without arming each case by hand. Per armed case: auto-resolve when the GC mirror shows a fresh ACTIVE mandate (they signed back up — messages stop by themselves); else send the due chase on each enabled channel (email via system Gmail — with the template's letter-before-action **PDF attached**; SMS via Trengo under the case owner's token — for a **standalone person** (no CRM contact) via `sendStandaloneMessage`, else the contact's thread), walking the staff-authored `DdRecoveryTemplate` sequence so each step is more serious; when the sequence is exhausted, stop + flag for a human. Never sends without the staff-pasted re-signup link. Every send logged (`DdCaseMessage` + timeline note when a contact is linked + audit). |
...
| `sales_executive` | Sales Executive | **Everything operational** (operator decision, 2026-07): all contact/family/interaction CRUD + merges, board & pipeline management (create/rename/reorder/archive stages + boards, quick actions, card delete), comms (email/Trengo/SMS/WhatsApp) send + assign, payment links, **refunds**, **subscription cancellation**, GoCardless money actions & DD recovery, invoicing raise/send/mark-paid, webinars/camps/complaints/labels/quick-replies/forwarding/branding/companies, reports. The ONLY things off-limits are the two locked buckets — Integrations and User management (see the callout below). |
| `virtual_assistant` | Virtual Assistant | **Identical capabilities to Sales Executive** (operator decision, 2026-07). "Virtual Assistant and above can do anything operational" — the two roles differ only in name/label, not in what they can do. |

> **Operational = every staff role; only Integrations + Users are locked (2026-07).**
> By operator decision "Virtual Assistant and above can do anything operational."
> `sales_executive` and `virtual_assistant` share one identical capability set —
> `SALES_ROLE_ACTIONS` in `packages/core/src/auth/policies.ts` — and every
> operational router role-Set (finance ops, GoCardless money actions, DD
> recovery, board/pipeline management, business-account & invoicing writes,
> webinars/camps/complaints, labels/quick-replies/forwarding/branding/companies,
> reports, contact merges & soft-delete) now includes all five roles. There is
> no "Manager+" wall on operational work any more.
>
> **The two locked buckets stay admin-only:**
> 1. **Integrations** — connecting/configuring external services (Slack, Trengo,
>    Gmail/email accounts, GoCardless, Zoom credentials, invoicing credentials,
>    booking, lead-source API keys, Slack channel routing), provider history
>    **imports/backfills**, feature flags, `secrets.rotate`, `tenant.config.write`.
>    These keep their existing CEO/Senior-Manager (or CEO/SM/Manager) gates —
>    e.g. `IMPORT_ROLES`, `CONFIG_ROLES`, `ZOOM_CONNECTION_ROLES` (split out of the
>    operational webinar `MANAGE_ROLES` so widening it never leaked Zoom connect),
>    `admin.integrations.*`, `admin.backfill.*`, `admin.flags.*`.
> 2. **User management** — accounts (`admin.users.*`), custom roles &
>    permissions (`role.*`), teams (`team.*`). Account creation is still capped by
>    `canCreateUserAtRole` and changing an existing user's role (`assignRole`)
>    stays CEO + Senior Manager, so this is not a privilege-escalation path. Also
>    kept restricted: `dsar.export` (GDPR bulk export), `audit.read`, and the two
>    genuinely destructive nukes — `card.clearBoard` (CEO-only, puzzle-guarded)
>    and permanent user erase.
>
> The safety rails that protect the data (idempotency, audit on every sensitive
> write, human-confirmed money moves, fail-closed enums — §2) are unchanged and
> apply to every role.
...
- Do not auto charge cards, auto delete data, or auto send messages. AI suggests, humans confirm. (The one exception is auto-merging duplicate contacts — operator-authorised in ADR 0047 and widened 2026-07 to merge EVERY duplicate cluster, sharing an email or a phone, with no human step; the `CONTACTS_AUTO_MERGE=off` kill-switch is the only control.)
...
| Change the home dashboard (`/`) | Page `apps/web/app/(app)/page.tsx` + skeleton `loading.tsx`; one round-trip `dashboard.summary` (`apps/web/app/api/trpc/routers/dashboard.ts`) returns role-aware **KPI tiles** (at-risk customers · customers — the Trengo "unassigned conversations" tile was removed 2026-07 at the operator's request) + a **"Needs attention"** action-queue grid (missed calls · leads · complaints · Slack mentions · DD issues — the DD-issues queue is visible to every staff role (2026-07) and honours the go-live cutoff `DD_ISSUES_CUTOFF_DATE`, default 2026-07-01, §9) + the live **at-risk-customers** list (derived hours risk, §6.4) + an "Explore the workspace" jump-to grid. **The Stripe finance-reconciliation surface was removed (2026-07):** the `/finance` UI section (overview / refunds / payment links / unresolved payments) and its dashboard queues (finance discrepancies, unresolved payments) + quick-link are gone; Direct Debits (`/direct-debits`) is the live money surface. Stripe is also no longer listed in Settings → Integrations. The finance tRPC router + `/api/webhooks/stripe` stay wired (forward-only, §19) so data still records. Queue assembly is the pure, unit-tested `apps/web/lib/dashboard/queues.ts` (`buildQueueCards`); components in `apps/web/components/dashboard/`. Missed-call + hours-risk counts reuse the shared core helpers (`projectCallInteraction` in `@studymind/core/calls`, `deriveHoursRisk` in `@studymind/core/contact`) and are bounded + resilient (degrade to "—" on failure). |
...
| Render an email body as rich HTML (Gmail-identical look, images + inline styles) | ADR 0041. The Gmail sync extracts the `text/html` part, sanitises + size-caps it (`prepareEmailHtml`/`sanitizeEmailHtml`, `packages/core/src/mail/html-email.ts` — strips script/iframe/handlers/`javascript:`, keeps inline styles + images), stores `payload.bodyHtml`. The `/mail` reading pane's `EmailHtmlBody` frames a **dedicated same-origin render route** `/api/internal/mail-render/[interactionId]` (staff-gated, `contact.get` access gate) inside a sandboxed iframe (no `allow-scripts`/`allow-same-origin`). That route returns the email document with its **own** CSP (`default-src 'none'; img-src https: http: data:; style-src 'unsafe-inline'`, no scripts) + `X-Frame-Options: SAMEORIGIN` + `Referrer-Policy: no-referrer`, + `upgrade-insecure-requests` (so `http://` image URLs aren't blocked as mixed content), so **remote images and inline styles render** while the app's strict nonce CSP stays intact. A `srcdoc` iframe (the previous approach) **inherits the parent's CSP**, which blocked both — that is why images didn't load. The route is excluded from the global strict CSP/XFO in `middleware.ts` + `next.config.mjs`. **When a message has no stored `bodyHtml`** (synced before HTML capture), the route **fetches it live from Gmail** via the owning mailbox's client and caches it back onto the Interaction — so images render for old mail too; the pane renders for every message carrying a `gmailMessageId`, not only ones with pre-stored HTML. `EmailHtmlBody` is a **shared component** (`apps/web/components/mail/email-html-body.tsx`) used by BOTH `/mail` AND the **contact page** Email section (`sections/EmailSection.tsx`) — the contact page previously showed only a "snippet" that was the SUBJECT repeated (the email view-model dropped `bodyHtml` and Gmail stores `summary=subject`), so emails read as having "no preview / no body"; now the collapsed row shows a real body-derived preview (`htmlToText(bodyHtml)`, `contact-channels.ts`) and expanding renders the full body via the same reading pane. Trengo-imported EMAILS are `type:'message'` (they show in the Trengo section, not here) and their raw-HTML summary is cleaned with `displayMessageBody` in the Activity `Timeline.tsx` so it is readable text, not tag soup. Inbound only today; HTML compose is a follow-up. |
...
| Work Direct Debit collections — chase underpayers / cancelled-early DDs, message them, recover (ADR 0045 + amendment) | **One merged `/direct-debits/issues` tab** (the old separate *Chasing* tab is gone; `/direct-debits/chasing` redirects here). Top: the **recovery-cases worklist** (`RecoveryCasesSection`) — each row opens the **`CaseDetailModal`**: identity, outstanding, **all previous communication** (`DdCaseMessage` history, auto + manual), **send a manual message** (email via system Gmail / SMS via Trengo, from a `DdRecoveryTemplate`, PDF auto-attached), and the **automatic-recovery** controls (re-signup link · email/text toggles · pause/resume · mark up to date). Below: the **detected-issue** sources (defaulters + `direct_debit_plan_shortfall`/`_arrears`) with a "Show issues before July 2026" reveal. **Every detected-issue row opens the SAME `CaseDetailModal`**, not a thin per-row control: a defaulter row via `cases.openCaseForFamily`, and a cancelled/underpaid or behind-schedule plan row via `cases.openCaseForSubscription` (find-or-create keyed on the plan's unique `gcSubscriptionId`, **identifies the person by onboarding/linking a CRM contact from the plan's GoCardless customer** when unlinked, else seeds the linked contact's email/phone, auto-send OFF on open §3). The row shows a compact at-a-glance chip (status + "Auto on · step N" / "Auto reminders off" / "needs link" + messages-sent count) from the automation fields `forSubscriptions` now returns; the old `ShortfallCaseCell` status/owner dropdowns + the contact-required "Send message" dialog (which dead-ended on the common unlinked cancelled plan) are gone. Inside the modal, **automated reminders are the headline**: a prominent Turn-ON/OFF switch + a one-line "On the automated reminder system — step N · next <date>" banner (`automationStatusLine`), a **recovery-goal toggle** (see below), the composer + history below; read-only staff see the status banner + history. A "Recovery templates & settings →" link sits in the Issues-tab header. **Detected issues auto-populate the worklist (ADR 0045 amendment):** the hourly `finance/flag-dd-defaulters` scan calls `autoOpenRecoveryCases` (`packages/jobs/src/finance/flag-dd-defaulters.ts`) which find-or-creates a `DirectDebitCase` for EVERY post-cutoff detected issue — cancelled/underpaid plans, plans behind schedule, AND family defaulters — so everyone who owes appears in the recovery list automatically and stays until a human resolves them (no "Start recovery" click needed; the manual open-case buttons remain for immediacy). **On-demand + self-hosted-safe (2026-07):** the same population + customer identification also run from a **"Scan for issues"** button on the Issues tab (`cases.scanNow`, Manager+), so the chase list fills without waiting for the nightly reconcile→scan cron (which self-hosted Inngest often doesn't deliver): it links unlinked GoCardless customers to their CRM contact (`linkUnlinkedGcCustomers`), recomputes the defaulter/underpaid/behind-schedule sets, auto-opens their cases, and **identifies + auto-onboards** any case still showing "Unknown" via `backfillRecoveryCaseContacts`. **No recovery case is ever "Unknown / no email / not in CRM" (2026-07 fix):** both auto-open AND the identify-backfill route through `resolveCaseIdentity` (`packages/jobs/src/finance/flag-dd-defaulters.ts`), which calls the shared **`resolveOrCreateContactForGcCustomer`** (`packages/core/src/finance/gc-onboard.ts`) — for a plan whose GoCardless customer isn't yet a CRM contact it (1) returns the already-linked contact, else (2) links an unambiguous email/phone match (attaching to the most-recently-updated on a shared identifier, an annotation not a merge §41.1, so the auto-merge cron collapses it), else (3) **creates a lightweight CRM contact from the GoCardless identity** (name + email + phone, `referralSource:'GoCardless'`, audited `contact.created` + `gocardless.customer.auto_linked`) and links it — the operator-authorised Direct-Debit analogue of the §10 call auto-onboard / §11 Trengo-import exception (a paid DD is a real customer, not spam). The case then carries the CRM contact's email/phone, and even in the rare no-contact path it **floor-fills the GoCardless name/email/phone** straight onto `personName`/`chaseEmail`/`chasePhoneE164` (fill-blank only — never clobbers a staff-set address). **A case is NEVER left "Unknown"** — `resolveCaseIdentity`'s display fallback is the GoCardless name, else the **plan name** (which usually embeds the person, e.g. "Lamar Fallatah 30h A-Level Tutoring"), so even a Direct Debit whose customer record is sparse and who was never in the CRM still shows a labelled entry. This runs **automatically every hour** — the on-demand "Scan for issues" button, the nightly `finance/flag-dd-defaulters` boundary (which now also `linkUnlinkedGcCustomers` first), AND a self-populating prelude on the hourly `finance/dd-chase-tick` cron (best-effort, never blocks chasing) — so the worklist stays populated + identified with zero button presses even when the reconcile→scan event chain never fires (the common self-hosted case). Because every detected issue becomes an identified recovery case, the raw **detected-issue tables are collapsed by default** as a "Detected-issue breakdown" disclosure (the same people, shown once) — expand for the collected-vs-contracted maths. An auto-opened case starts with no re-signup link, so it sends nothing until the **global** re-signup link is set; **automatic chasing is ON by default** (operator decision) so once that one link exists every new case is auto-armed and chased by the hourly engine (see §17.1 `finance/dd-chase-tick` auto-arm; `DD_AUTO_CHASE=off` pauses it). Staff can still configure/pause an individual case by hand and the engine never overrides that. **Recovery goal per case (`recoveryStrategy`, `resend_link`|`demand_full`):** the modal toggle "Sign up to a plan again" vs "Pay the full amount now" — `resend_link` needs the pasted link before the engine sends (`decideChaseTick` `no_link` skip); `demand_full` chases for the full outstanding balance and needs no link. **A case works WITHOUT a CRM contact** — most defaulters predate the CRM, so `openManualChase` accepts a standalone `personName` + email + phone (`DirectDebitCase.personName`, contact/customer links all nullable), and the engine + `sendCaseMessage` route standalone SMS through `trengoOutbound.sendStandaloneMessage` (raw phone, no contact Interaction — the `DdCaseMessage` is the record). Engine `apps/web/app/api/inngest/_boundary/dd-chase.ts` (hourly, **7-day cadence**); pure decisions `packages/core/src/finance/dd-chase.ts`; tRPC `finance.directDebit.cases.{chaseList,caseDetail,forSubscriptions,openManualChase,openCaseForFamily,openCaseForSubscription,updateChase,markUpToDate,chaseMessages,sendCaseMessage,draftMessage}` (reads all staff, writes Manager+). **Calculated CCJ costs:** the letters quote a real "if it goes to court" figure — England & Wales court issue fee (gov.uk EX50 scale) + 8% simple statutory interest (s69 County Courts Act 1984) — via `estimateCcjCosts` (`packages/core/src/finance/ccj-costs.ts`, verified July 2026) surfaced as `{{court_fee}}`/`{{interest}}`/`{{daily_interest}}`/`{{total_with_costs}}`/`{{late_fee}}`/`{{response_deadline}}` tokens; the shared var-builder (`apps/web/lib/finance/recovery-vars.ts`, `buildRecoveryVars`) feeds BOTH the engine and the manual-send preview so the customer sees one number. **Seeded sequence** (migration `20260719200000_seed_dd_recovery_templates`, idempotent): four escalating **email** templates + four matching **SMS/Trengo** templates, generalised (no personal names) + tokenised; the two `legal_escalation` email steps also attach a **generated PDF of the letter** (`renderRecoveryLetterPdf`, from the personalised body) plus any staff-uploaded accompanying PDF (`ddRecoveryTemplate.{attachPdf,removePdf}`, served at `/api/dd-recovery-templates/[id]/pdf`). Copy authored at **Settings → Direct Debit recovery** (`/settings/dd-recovery-templates`, Manager+) — the templates admin shows the sequence as two clear channel groups (**Email sequence** / **Text sequence**, each in step order, so the email+text pair per step no longer reads as duplication), plus a **Recovery settings** panel (`DdRecoverySettings` singleton, tRPC `ddRecoverySettings.{get,update}`) that makes the policy figures customisable — **late fee, cadence, response window, finance phone, letterhead** — read by the engine + sends via `loadDdRecoverySettings` (`apps/web/lib/finance/recovery-settings.ts`), env as fallback. **Automatic chasing (the master switch for hands-off reminders):** the same panel carries `autoChaseEnabled` + the single global **re-signup link** (`autoChaseSetupLinkUrl`) + email/text channel toggles; when on, the hourly engine auto-arms every new case (see the `finance/dd-chase-tick` row, §17.1). The Issues tab shows a one-line banner of the live state (ON / ON-but-no-link / OFF, `AutoChaseBanner`) so it's never a mystery why reminders are or aren't sending. Env fallbacks `DD_AUTO_CHASE`/`DD_AUTO_CHASE_LINK`. The court fee + statutory interest stay calculated in code (fixed by law). **Optional "Refine with AI"** (`cases.draftMessage` → `packages/ai` `dd_recovery_draft`, §3/§18) lightly personalises an already-filled draft while keeping every figure/link/legal statement verbatim; agent reviews before sending. Stops on: fresh active mandate (auto), "Up to date ✓" (manual), pause, or sequence exhaustion (flags for a call). |
...
| See who viewed / edited / deleted a record (audit activity) | tRPC `audit.forTarget` (per record) + `audit.list` (org-wide) + `audit.recordExport` (`apps/web/app/api/trpc/routers/audit.ts`, gated `audit.read`). View-model `apps/web/lib/view-models/audit-activity.ts`. UI: per-contact + per-account **Activity log** (`sections/ActivitySection.tsx`) and the top-level **Audit log** nav section (`app/(app)/audit/`). Record opens are captured as `contact.viewed` / `business_account.viewed` in the respective `get` procedures; CSV exports as `contact.exported`/`account.exported`. |
| Permanently erase a contact (GDPR Article 17) | Core `eraseContactData` (`packages/core/src/compliance/erase-contact.ts` — crypto-shred + anonymise, idempotent, audited `contact.erased`). tRPC `contact.{erase,scheduleErasure,cancelErasure}` (CEO/SM, retype-confirm). Schema `Contact.erasureScheduledAt`/`erasedAt`. Daily completion cron `compliance/erase-due-records` (§17.1). UI: `EraseContactButton.tsx` on the contact page. Immediate or 30-day grace. |
...
| Create a full-admin account from the CLI (nobody can sign in yet) | `pnpm create-admin <email> [name]` → `packages/db/prisma/create-admin.ts`. Creates or promotes ANY email to a canonical role (default `ceo`), generating a strong temporary password and printing it once; forces a first-login change and audits `auth.user_created` + `auth.role_granted`. **Non-destructive + idempotent** — re-running an existing user only ensures the role assignment; a password is overwritten ONLY behind `ADMIN_FORCE_PASSWORD_RESET=true` (§3), which also clears lockout/deactivation. Distinct from `pnpm seed:super-admin`, which bootstraps the ONE canonical CEO row from `SUPER_ADMIN_*` env on deploy. Runbook `docs/runbooks/create-admin-account.md`. Prefer Settings → Users (row above) whenever someone can already sign in. |
...
| Pick purchases up from payment-alert emails + auto-enrol (no Stripe API, ADR 0048) | Sender gate `packages/integrations/gmail/src/purchase-email.ts` (`isPurchaseAlertSender`, `PURCHASE_ALERT_SENDERS`) wired into `processMessage` behind the `stripe.purchase_email_ingest_enabled` flag → fires `webinar/purchase-email.received`. Boundary `apps/web/app/api/inngest/_boundary/purchase-email.ts`: AI extract (`purchase_email` task, `packages/ai/src/prompts/purchase-email.ts`) → match/create buyer contact → record a `payment` Interaction (`source:'stripe_email'`) → `enrollFromPurchase` (`apps/web/lib/webinar/enrollment-service.ts`). Confident weekly-class matches auto-enrol, rest `pending_review` (§47). Record-keeping + service provision only — no charge/refund/invoice. Off until the mailbox is connected + the sender is set. |
...
| Clear a whole board (archive every card in one action) | `clearBoardCards` (`packages/core/src/board/cards.ts`); tRPC `card.clearBoard` (**CEO only**, `BOARD_CLEAR_ROLES`) — **double-verified**: the button (`ClearBoardButton.tsx`, shown only to CEO) opens a modal requiring an arithmetic **puzzle** AND typing the board's exact name, and the server independently re-checks the typed `confirmation` against the board name. Soft-archives all live cards (contacts + history untouched), one `board.cleared` audit row. |
...
| Board kanban scrolling / density (Todoist-style) | `BoardDnd.tsx`. The board is a fixed-height (`h-[calc(100dvh-13.5rem)]`) row that scrolls **only horizontally**; **each column scrolls its OWN cards vertically** (`BoardColumn` card `<ul>` is `data-col-scroll` `overflow-y-auto min-h-0 flex-1`) with a fixed header + add-card footer — so nothing is clipped (the old model stretched columns and `overflow-hidden` hid overflowing cards, the "can't scroll down" bug). Always-present scroll controls frame the board: horizontal ◀▶ `HScrollBar` fixed at the **top and bottom** (`scrollColumns`), vertical ▲▼ `VScrollRail` pinned on the **left and right** edges (`scrollVertical` scrolls every column's list in parallel). Controls show when that axis overflows (`overflowsX`/`overflowsY`, `ResizeObserver` + `MutationObserver`). Denser + zoomed-out: columns `min-w-[248px] max-w-[264px]`, `gap-3`, card `p-2.5` — many more cards/columns visible at once. |
...
| Manage lead-source API keys (Contact Form 7 sites) + route a site to a board | Settings → Integrations → Lead webhook (`LeadIngestionPanel`, Manager+). tRPC `lead.sources.*` (list / create / update / rotate / archive / **delete**); schema `LeadSource` (sha256 key hash + last4, optional pinned brand, optional **`targetBoardId`**). Raw key shown once. **Per-source board routing:** the Board column pins a site's enquiries to a pipeline (e.g. the ANZ website's CF7 key → the **ANZ Sales Pipeline** board, `board_seed_anz_sales`); the routing lives in `resolveLeadDestination` (`packages/jobs/src/leads/process-lead.ts`) with precedence **free-resources classification > source targetBoard > default Sales board**, each falling back so a lead is never lost (`targetBoard` SetNull FK). `delete` hard-removes the key; past leads preserved (`Lead.sourceId` `onDelete: SetNull`). `archive` is the reversible soft-disable. |
| Card sub-tasks (Todoist-style checklist on a card) | Schema `CardSubtask` (`cardId`, `title`, `completed`, `position`). Domain `packages/core/src/board/subtasks.ts`; tRPC `card.subtasks.*` (list / add / update / delete, Sales Executive+). UI `apps/web/app/(app)/boards/[boardId]/CardSubtasks.tsx` in the card modal. A card-local checklist; the standalone CRM Tasks feature was removed (ADR 0046) — this is unrelated and stays. |
...
| Edit a wrong phone number inline (card / contact / list) | The `PhoneLink` chevron menu (`apps/web/components/shared/channel-links.tsx`) gains an **Edit number** item when passed a `contactId` — an inline `PhoneInput` that saves via `contact.update` (audited). Wired into the board card, board list view, card sidebar, contact-page header, and the Contacts + at-risk tables. |
| Fix the wrong numbers already imported (Invalid number board) | "Fix wrong numbers" button on Settings → Integrations → Lead webhook (`LeadPhoneRepairButton.tsx`, Manager+). tRPC `lead.phoneRepair.{preview,apply}`; scan `apps/web/lib/leads/phone-repair.ts` (`scanInvalidNumberRepairs`, `board_seed_invalid_number` cards); pure per-contact decision `proposePhoneRepair` (`packages/core/src/lead/phone-repair.ts`, unit-tested) — re-derives from `Lead.rawPayload` with the corrected resolver, confident (form/phone-dial) + changed only. Preview → apply, audited `contact.updated`, idempotent. |
| Store multiple emails / phone numbers / other points of contact on a Contact | Schema `ContactChannel` (kind `email\|phone\|other`, `value`, optional `label`, `sortOrder`, soft-deleted). The primary `Contact.email`/`phoneE164` stay the **matching source of truth** (lead dedup §16, call matching §10); these are supplementary. tRPC `contact.points.{list,add,update,remove}` (audited `contact.point_*`; any staff writes, mirrors `contact.update`); pure validation `packages/core/src/contact/points.ts`. UI: **"Points of contact"** section on the contact page (`sections/PointsOfContactSection.tsx`) — primary shown read-only, extras add/edit/remove. The board **Add-card** form also captures extra numbers ("Add another number" → saved as `phone` points on the new contact). **Matching honours the extra channels** on the two auto-onboard paths: an inbound **call** to a 2nd/work number links to the right contact (`resolveOrCreateContactForCall`, §10), and a **web re-enquiry** from a 2nd number/email attaches to the existing contact instead of duplicating (`process-lead.ts` byEmail/byPhone OR a `ContactChannel` match). The Slack/call-summary `matchContactByCandidate` still keys on the primary only — a tracked follow-up. |
| Bulk-merge duplicate contacts | `/contacts` table → select 2+ → Merge (Manager+). tRPC `contact.bulkMerge` ({survivorId, loserIds}); first selected row is the survivor, the rest merge in via `mergeContacts` (`apps/web/lib/services/contact-merge.ts`) one at a time. **Auto-find duplicates** at `/contacts/duplicates` (Manager+): `contact.duplicates.find` clusters contacts sharing a normalised email or last-9-digit phone (union-find, `packages/core/src/contact/duplicates.ts`, unit-tested), one-click Merge per cluster. **Fully-automatic merging (ADR 0047; widened 2026-07):** the hourly `contacts/auto-merge-duplicates` cron (+ a Manager+ "Run auto-merge now" button → `contact.duplicates.autoMergeNow`) merges **every** duplicate cluster with no human step — `planAutoMerges(rows, { includeAmbiguous: true })` merges each cluster (any contacts sharing a **normalised email** or a **last-9-digit phone**) into its **oldest** member. This now INCLUDES a phone-only match with different names (a possible shared family landline, §41.1) — by operator decision nothing is parked for manual review. Merges run through the audited `mergeContacts` with a system actor and skip (never fail) restricted-access conflicts; shared logic in `apps/web/lib/services/auto-merge-duplicates.ts` (bounded `maxMerges` chunk for the interactive path), cron boundary `apps/web/app/api/inngest/_boundary/auto-merge-duplicates.ts` (disable with `CONTACTS_AUTO_MERGE=off` — the only human control). **Inngest-independent self-drain + ZERO manual UI (2026-07):** because self-hosted Inngest often doesn't fire the hourly cron (so the backlog sat, and the page wrongly asked for a manual merge), opening `/contacts/duplicates` now runs the SAME merge **synchronously in the request** — `contact.duplicates.drainNow` loops `runAutoMergeDuplicates` in bounded ~400-merge chunks and the page auto-loops it on mount until a pass merges nothing (drained, or only an unmergeable restricted-access conflict remains), then shows a status-only "all clear". No button, no Inngest needed; the cron stays as the background backstop. **`/contacts/duplicates` has NO manual merge UI at all** — it is status-only (operator direction 2026-07: "0 manual stuff"). `drainNow` respects the `CONTACTS_AUTO_MERGE=off` kill-switch: when paused the page just reports "automatic merging is turned off" (no merge queue); when a group genuinely can't be auto-merged (a restricted-access safeguarding conflict, §41.1) it shows a read-only note that they're kept separate by design — never a merge button. The explicit `contact.duplicates.autoMergeNow` trigger deliberately ignores the kill-switch (a human asked to merge) and is retained as an API but no longer surfaced. Explicit hand-merging, if ever wanted, lives on the `/contacts` table's **select 2+ → Merge** power-user tool (below), not on the duplicates page. Pass no options to `planAutoMerges` for the legacy confident-only behaviour. Lead ingestion also dedupes format-insensitively (last-9 phone suffix + case-insensitive email, `buildPhoneMatch`) so re-enquiries never create a new dupe. |
...
| Record a call summary (typed → CRM record + Slack) | Both the **contact page** (`apps/web/app/(app)/contacts/[id]/sections/CallSummarySection.tsx`, `contact.callSummary.add`) and the **board card modal** (`apps/web/app/(app)/boards/[boardId]/CallSummarySection.tsx`, `card.callSummary.add`) render the shared form `apps/web/components/contact/call-summary-wizard.tsx`: a staff member picks an optional outcome (answered / voicemail / no answer), types the summary, and submits. It is **recorded on the customer's CRM record** as a `call_summary` Interaction (`addContactCallSummary` / `addCallSummary` in `packages/core/src/{contact,board}/call-summary.ts`) and **announced to the operator-routed `#callsummaries` Slack channel** (resolved via the `call_summary` topic, ADR 0033) — best-effort, so a Slack failure never loses the record. **No customer message is ever sent from the CRM** (no email, WhatsApp/SMS, templates, PDFs, or VA task hand-off — the whole flow was simplified in the 2026-07 redesign, ADR 0046). The Slack post (headline outcome + name + phone + email, summary body, `logged by`) is the pure `buildCallSummarySlackBlocks` (`packages/core/src/slack/blocks.ts`); the sender is `postCallSummaryToSlack` in `apps/web/lib/board/call-summary-senders.ts`. The `/call-summaries` workspace (row below) is the third surface. **Slack-posted call summaries are recognised by CONTENT, not just channel:** a Slack message that IS a call summary — recognised by the pure `looksLikeCallSummary` (`packages/core/src/slack/call-summary-detect.ts`): a call-summary channel (name contains `summar`, minus complaint channels), OR the labelled call-log format ("Client Name and Number / Client Email / Summary of call / Actions"), OR explicit call-outcome language ("voicemail", "no answer", "called twice", "call completed", "rang out") — surfaces in the contact's **Call Summaries feed** (`source:'slack''`), and a plain mention (no call signal) stays in the **Slack mentions** section only. The two are mutually exclusive: `callSummariesForContact` includes a `slack_summary` iff `looksLikeCallSummary`, `slackMentionsForContact` excludes those, and the KPI tiles count the same partition (all content-based, read-time, so it's fully retrospective over existing rows). Complaint-channel posts are Complaints, never call summaries. **Smart assignment (content):** every ingest path (live/backfill/relink) prefers the labelled format's explicit CLIENT identity — `parseCallLogClient` ("Client Name and Number / Client Email", complaint-field-optional, unlike `parseStructuredComplaint`) — over the first email/phone in the text (which can be the guardian), so a call summary links to the right customer. |
...
| Archive Slack messages about a customer (survives 90-day retention) | ADR 0034. The Slack parser (`slack/event.received` + 90-day backfill) only ingests **human-authored** messages — bot/app posts (incl. the CRM's own compulsory `#callsummaries` announcements, ADR 0039) are skipped by the pure `isIngestableSlackMessage` (`packages/integrations/slack/src/message-filter.ts`, needs `bot_id`/`app_id` on the event) so we never re-ingest our own post as a duplicate `slack_summary` (§3). It deterministically matches on an email/phone in the text first (free, AI-independent), else AI-classifies (mini tier) and, on a match above 0.5 confidence (email → normalised phone → unambiguous name incl. **single token / surname**, `matchContactByCandidate`), writes a `slack_summary` Interaction on the Contact storing the **original message** (`messageText`, `senderName`, `channelId`, `permalink`) so the record outlives Slack's retention, plus an AI **category** (`billing·scheduling·feedback·complaint·academic·logistics·sales·general` — `slackSummarySchema`). Surfaced on the contact page Slack section (category chip + original text) via the `SlackMention` view-model. Unmatched/ambiguous references are recorded in `UnassignedSummary` and surface in the **triage tray** at `/inbox/slack-mentions` (also on the home dashboard "Needs attention" queue) — **on all three ingestion paths (webhook, 90-day backfill, recurring/on-demand pull), which share `processSlackMessage`; the backfill/pull previously dropped unmatched messages silently, the reason imported history "didn't show up"**: tRPC `slackSummary.unassigned.{count,list,assign,dismiss,bulkDismiss}` (Sales Executive+ to triage). **Zero-queue by construction (2026-07):** the ingest itself no longer leaves an OPEN row — all three paths call the shared `finalizeUnresolvedMention` (`packages/integrations/slack/src/finalize.ts`), which in full-auto (default) records the archive AND resolves it (auto-dismiss) in the SAME write, so a NEW un-matchable mention never enters the human queue (the fix for "new mentions keep being added to the file for someone to assign by hand"). Only the legacy open backlog needs draining, which the cron + the tray-open `drainNow` clear. **Self-heal (2026-07):** a dismissal that still carries an identity signal (a name / email / phone the matcher couldn't resolve to a contact yet — usually because that customer isn't in the CRM) is flagged `UnassignedSummary.autoLinkPending`; a re-link pass (`relinkAutoDismissedBatch`, run by the cron, the `slack/relink-now.requested` event **fired on `contact.create`**, and the tray-open `drainNow`) re-runs the matcher over those (MATCH-ONLY, never onboard, no complaint raised) and files the mention on the customer's timeline the moment they are added to the CRM, clearing the flag — the row stays resolved throughout so it never re-enters the human tray. Pending rows expire off the retry set after `AUTO_LINK_HORIZON_MS` (365d) to stay bounded. The `/inbox/slack-mentions` page copy now explains it is fully automated and only a rare safety-net view. `assign` files the mention to a customer **Contact OR a school/B2B account** (exactly one; a contact also stamps its primary school), so a parked org mention can be filed to the school by hand; `bulkDismiss` clears a multi-select of noise in one action (tray checkboxes). **The tray is a fully-automated self-draining system (default, `slackTrayFullAuto`, operator direction 2026-07 — no human queue):** the `slack/relink-unassigned` cron (§17.1) re-runs the matcher over parked rows for free and (a) auto-links / auto-onboards every resolvable one and (b) **auto-dismisses the rest** (`resolveUnlinkedOutcome`) — unrescuable rows always, and in full-auto mode a substantive-but-nameless row too (dismiss = `resolvedAt` set, retained + reversible, never deleted) — so the tray drains to zero without any human approval. **Inngest-independent auto-drain (2026-07):** because self-hosted Inngest often doesn't fire the cron (so the backlog sat forever), opening `/inbox/slack-mentions` now runs the SAME link-or-dismiss logic **synchronously in the request** — `slackSummary.unassigned.drainNow` calls `relinkParkedRowsBatch(..., { threadFetches: 0, forceFullAuto: true })` (thread-parent fetches off; the channel-name `conversations.info` lookup is BOUNDED to `RELINK_SYNC_CHANNEL_NAME_API` distinct channels/batch, cached — enough to unlock call-log name-only onboarding for legacy null-channelName rows without ever stalling) in bounded ~1000-row chunks (10 batches/call, per-batch try/catch so one failing batch can't abort the drain), and the tray auto-loops it on mount until `remaining` is 0. No button, no Inngest needed; the cron stays as the background backstop. **`forceFullAuto: true` overrides the `SLACK_TRAY_FULL_AUTO` kill-switch** so an operator opening the tray ALWAYS clears it to zero regardless of how the flag is set — a mis-set flag can never be the reason a legacy backlog persists (the "553 still waiting" bug). `SLACK_TRAY_FULL_AUTO=off` reverts to keeping a substantive nameless / ambiguous row for a human (§3). Three matcher fixes make (a) actually drain: the relink now **filters own-brand names** (`isOwnBrandName`) before the deterministic name pass exactly as the live webhook/backfill do — the missing filter let "… Medic Mind" resolve as a second entity and park otherwise-linkable mentions (the "Paula Baker not linking" bug); `resolveSlackLinkTargetFromNames` now **collapses to a single resolved person** when a message names one customer plus their school / an unresolvable co-name (parks only on 2+ different people); and matching stays exact/unambiguous-only otherwise. The webhook `GET` reports `aiConfigured` so it's obvious when a missing AI key is why name-only mentions aren't extracting. Never auto-create / auto-match on ambiguity (§12, §3). |
| Log / work a complaint (CRM-logged, posts to Slack, threaded) | Complaints hub `/complaints` (queue + Active/Assigned-to-me/Resolved/Archived/All filters + KPI strip) → complaint detail `/complaints/[id]` + the contact page's Complaints section — both render the shared **`apps/web/components/complaints/ComplaintDetailPanel.tsx`**. Log dialog `NewComplaintDialog.tsx`: **Find in CRM** (`ContactFinder`) OR **Enter manually** (name + phone + email); `complaint.create` stores `contactId` OR `personName/personPhone/personEmail` (`Complaint.contactId` nullable), and **ALWAYS posts to `#complaintcallsummaries`** (`postComplaintToSlack` → `complaint_call_summary` topic, `apps/web/lib/complaints/slack-sender.ts`) + anchors the thread (`slackChannelId/slackMessageTs`). **Routing is name-based + self-healing (2026-07 fix):** the sender resolves the destination via `resolveTopicChannelWithDiscovery` (`apps/web/lib/slack/topic-channel.ts`), which — when no explicit `complaint_call_summary` route is configured — **auto-discovers the Slack channel literally named `complaintcallsummaries`** (tolerant match via `normaliseSlackChannelName`, the topic's `defaultChannelName`), **wires it up** (persists a `SlackChannelOption` + `SlackRoute` so Settings shows it and later sends skip discovery), and posts there — instead of silently falling into the generic default/`SLACK_ALERTS_CHANNEL_ID` channel (the "complaints not reaching my channel" bug). The **REAL** channel is stored (`Complaint.slackChannelName`) and reported in the log toast + the detail panel — never a hardcoded guess — and a not-in-channel/failed post is surfaced (invite-the-bot guidance), not swallowed. Falls back to the default channel only when discovery can't run (no `channels:read` scope), so the misroute is at least visible. Thread updates (`addUpdate`) log a `note` on the contact timeline (if linked) AND mirror into the Slack thread (`postComplaintThreadReply` → `postAlert({threadTs})`). Full lifecycle in the router (`apps/web/app/api/trpc/routers/complaint.ts`): `update` (status/severity/category/assignee/title/description), `resolve/reopen`, `archive`, `delete/restore` (Manager+), `permanentlyDelete` (CEO/SM), `assignableUsers`. Pure customer resolver `apps/web/lib/complaints/customer.ts` (`complaintCustomer`, unit-tested); Slack builders in `packages/core/src/slack/blocks.ts`. **Slack→complaint auto-ingestion is REMOVED** (see §12) — no message opens a complaint. |
| Route which Slack channel each kind of notification goes to | `/settings/slack-channels` → "Where notifications go" (Manager+, ADR 0033). Schema `SlackRoute` (`topic` unique → `SlackChannelOption`, `enabled`); code registry `SLACK_TOPICS` (`packages/core/src/slack/topics.ts`: call_summary, **complaint_call_summary**, google_voice, finance_dd_defaulters, cost_summary, security_alerts, general_alert). Most senders call `resolveTopicChannelId(db, topic, fallbackEnvChannelId?)` (`packages/core/src/slack/route-resolver.ts`): route → env fallback → default option → `SLACK_ALERTS_CHANNEL_ID`; `enabled=false` mutes. **The call-summary + complaint senders instead use `resolveTopicChannelWithDiscovery` (`apps/web/lib/slack/topic-channel.ts`)** — same order, but when a topic carries a `defaultChannelName` (`call_summary`→`callsummaries`, `complaint_call_summary`→`complaintcallsummaries`) and has no explicit route, it **auto-discovers that channel by NAME and wires it up** (persists option+route) so the message reaches the intended channel with zero config, and returns the real channel name for honest UI. tRPC `slackChannel.routes.{list,set}`; wired into the Google Voice handler + the dd-defaulters / cost-summary / UEBA boundaries. Add a new routable *kind* = add to `SLACK_TOPICS` (code, optionally with a `defaultChannelName`); re-routing an existing one = no code. |
...
| External app links (Main Portal / Invoices / HR / Trengo) | Catalogue in `apps/web/components/shell/external-links.ts`, env-overridable (`NEXT_PUBLIC_MAIN_PORTAL_URL` / `NEXT_PUBLIC_INVOICE_SITE_URL` / `NEXT_PUBLIC_HR_SITE_URL` / `NEXT_PUBLIC_TRENGO_URL`; defaults `portal.studymind.co.uk`, `b2b.studymind.co.uk`, `hr.studymind.co.uk`, `app.trengo.com`). Rendered in the TOP BAR (md+, `top-bar.tsx`); the sidebar "External" group only shows in the mobile drawer (`sidebar-nav.tsx`, `md:hidden`). |
...
| Export a list to CSV | `<CsvExportButton>` (`apps/web/components/ui/csv-export-button.tsx`) + `apps/web/lib/csv.ts` (RFC 4180 escape, UTF-8 BOM so Excel auto-detects). Mounted on `/accounts` and the InvoicesPanel header. List pages page through their tRPC procedure in 100-row chunks (cap 5000) so the export honours the current filter state. |
| Export B2C **customers** to CSV (comprehensive · filtered or all) | `/contacts` header → **Export CSV ▾** dropdown (`apps/web/app/(app)/contacts/ContactsExportMenu.tsx`), two scopes: **Export current view** (honours every active filter — search + all facets + ranges) and **Export all customers** (filters ignored). Both stream the dedicated **`contact.exportRows`** query page by page (keyset cursor on `(createdAt desc, id desc)`, 200/page, cap 50 000) — a **comprehensive** row: full profile + address, DOB/minor, points of contact (extra emails/phones/other from `ContactChannel`), education (school/year/SEND/exam target), preferences, companies/labels/subjects/enquiry types, family, booking status + engagement figures (hours booked/done/remaining, spend, last lesson), comms counts, active complaints, derived hours-risk, last-contacted, notes, created/updated. The filter shape + `where`-builder (`ContactFilterShape` / `buildContactFilterWhere`) are **shared** with `contact.list`, so "current view" always matches the on-screen table. Row builder `toContactExportRow` (`apps/web/lib/view-models/contact.ts`, unit-tested); every run audits `contact.exported` with a scope/filter summary (§20/§21). |
...
| Sync Summer Camp bookings + view live camp roster/timetables (`camp.studymind.co.uk`) | `packages/integrations/summer-camp/` — **inbound (live)** `webhook.ts` (HMAC verify) + `apply.ts` (match/create guardian `parent` + attendee `student` Contacts, reciprocal `parent_of`/`child_of` link, `booking` Interaction on each timeline, sales-pipeline card, audit `summer_camp.booking.*`) behind `jobs.ts` (`summer-camp/event.received`); receiver `apps/web/app/api/webhooks/summer-camp/route.ts` (secret `SUMMER_CAMP_WEBHOOK_SECRET`). **Backfill + periodic sync (pull)** `jobs.ts` `summer-camp/backfill-bookings` (admin button on `/camps`, CEO+SM — walks `GET /api/external/bookings` to import all current bookings) + `summer-camp/sync-bookings` (cron, §17.1 — safety net). Pull path applies via the same idempotent `applyBookingEvent` (`audit:false`, summary audit only). **Outbound read feeds** via `client.ts` (`SUMMER_CAMP_API_URL`/`_API_KEY`, null when unset), tRPC `summerCamp.{status,camps,timetable,backfill}`, UI `apps/web/app/(app)/camps/` — **year-scoped** (the camp feed scopes camps to the requested season year and returns `available_years`; the CRM renders year chips on Overview + Schedule, and the Schedule is organised YEAR → CAMP → DAY with camp chips + day-jump anchors instead of every camp stacked). The whole `/camps` section shares a segmented `CampsNav` (Overview · Bookings · Schedule · Stripe purchases · Instalments), `StatTile` strips, and one tonal status-pill map (`camp-status.tsx`). **Local record (`CampBookingRecord`)**: every sync path (webhook, backfill, 15-min cron, CRM-originated create) upserts the CRM's own durable copy of each booking, linked to the student/guardian Contacts — the bookings workspace reads THIS table (survives camp-app downtime); detail editing is a SlideOver. **Two-way write-back** (`writeback.ts`): a CRM note or identity-field edit on a camp-linked contact is pushed to the matching camp booking (`PATCH /api/external/bookings/:id` + `/notes`), hooked best-effort into `interaction.create` + `contact.update`; loop-safe (camp tags writes `system:crm`, doesn't echo; feed marks them `source:'crm'` so the CRM skips its own note). **Bookings workspace** (`/camps/bookings`, tRPC `summerCamp.bookings.{list,update,assignCamps,addNote}`): the per-booking surface — rows come LIVE from the camp's keyset feed (filtered via the pure `bookings-filter.ts`), and staff edit status/subject/booking-notes (`update` — Sales Executive+, cancellation Manager+), **assign the student to camps** (`assignCamps` → `PUT /api/external/bookings/:id/camps`, first id = primary, mirrors the camp's `student_enrolments`; the feed's `enrolled_camp_ids` field carries the full assignment back), and add shared notes (`addNote`, any staff — keyed on the camp's note id so feed round-trips never duplicate). Every write goes to the camp FIRST, patches the mirrored `booking` Interactions + `CampBookingRecord`, audits (`summer_camp.booking_*_from_crm` / `booking_camps_assigned` / `booking_note_added`, §45), and fires `summer-camp/sync-bookings.requested` to converge in seconds. **Stripe purchase pipeline (`/camps/purchases`)**: the `charge.succeeded` job runs `detectCampPurchase` (`packages/core/src/camp/purchase-detect.ts`, unit-tested — conservative "summer camp"/"work experience" phrase match on the charge's product text) and fires `summer-camp/purchase.detected`; the worker records a `CampStripePurchase` (idempotent on the charge id) and **auto-creates the camp booking through the CRM** via the camp app's `POST /api/external/bookings` (idempotent on `payment.reference` = the charge id; no camp picked → the camp's `camp_triage` queue + our "Needs camp" filter), then applies it locally. Failures (camp not connected, no billing name) stay `pending` in the tray with Retry/Dismiss (Sales Executive+); `summer-camp/scan-purchases.requested` (CEO/SM button) walks recent Stripe charges historically. This is the one deliberate exception to "the CRM never creates a camp booking" — it enters REAL paid purchases, never invents one. Distinct from `booking.studymind.co.uk` (the tutoring platform). Contract: the camp repo's `CRM_INTEGRATION.md`. |
...
| Work in the internal team chat (Slack-style) | `/messages` (ADR 0022). Domain `packages/core/src/chat/` (channels, messages, mentions, reactions, refs, read-state; the client-safe body grammar is `chat/parse.ts`, imported via `@studymind/core/chat/parse`). tRPC `chat.*` (`apps/web/app/api/trpc/routers/chat.ts`). UI in `apps/web/app/(app)/messages/`. Channels (public/private), DMs, threaded replies, @mentions, emoji reactions, and inline `<~type:id>` references to Contact/Family/Card (a legacy `task` ref type is retained forward-only for old bodies but no longer offered/resolved — ADR 0046). Channel admin is Manager+ and audited (`chat.channel_*`, `chat.member_*`); messages are staff↔staff and deliberately NOT written to the customer timeline or the compliance AuditLog. |
...
- E.164 phone numbers are unique per `Contact` row but may legitimately repeat across a Family (shared landline). **As of the 2026-07 operator decision (ADR 0047, §37), a phone-only match — even with a different name — IS auto-merged** along with shared-email and shared-phone+name matches: the auto-merge cron runs `planAutoMerges({ includeAmbiguous: true })`, so every duplicate cluster is combined with no manual review. (The pure `planAutoMerges` default still models the old confident-only rule for any other caller; the shared-landline caveat is retained here as the rationale for that default.)
