...
| See who viewed / edited / deleted a record (audit activity) | tRPC `audit.forTarget` (per record) + `audit.list` (org-wide) + `audit.recordExport` (`apps/web/app/api/trpc/routers/audit.ts`, gated `audit.read`). View-model `apps/web/lib/view-models/audit-activity.ts`. UI: per-contact + per-account **Activity log** (`sections/ActivitySection.tsx`) and **Settings → Audit log** (`settings/audit/`). Record opens are captured as `contact.viewed` / `business_account.viewed` in the respective `get` procedures; CSV exports as `contact.exported`/`account.exported`. |
