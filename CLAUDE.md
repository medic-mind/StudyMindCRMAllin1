...
| Custom labels for B2B accounts (schools + partners) | `/settings/account-labels` (Manager+) curates a shared, colour-coded catalogue. Schema `AccountLabel` (name unique, `color`, `description`, `sortOrder`, `archivedAt`) + `BusinessAccountLabel` (M:N junction, cascade). tRPC `accountLabel.{list,pickList,create,update,archive,attach,detach}` (list/pickList any staff; CRUD Manager+; attach/detach Sales Executive+). Applied in bulk from the Accounts list and surfaced as chips on each row. Distinct from `Label` (board cards, ADR 0018) and `Company` (brand tags). |
