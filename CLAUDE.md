...
Anything below threshold, name-ambiguous (two Jane Smiths), or unmatched lands in the "unassigned summaries" tray — but that tray is no longer a dead-end: the recurring **`slack/relink-unassigned`** job (§17.1) re-runs the resolver over every parked row for free and auto-links the unambiguous ones (draining the backlog + self-healing when a contact/account is created after the mention), and staff can force it immediately with the tray's **"Re-run matching now"** button (`slackSummary.unassigned.relinkNow` → `slack/relink-now.requested`, Manager+). Never auto-attach on ambiguity, never auto-create a contact or account (§3).
