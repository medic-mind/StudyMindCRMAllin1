...
| Open the dedicated email workspace | `/mail` (ADR 0021 Phase 4, `apps/web/app/(app)/mail/page.tsx` shell → `MailWorkspace.tsx` client). **Gmail-class layout**: an icon folder rail (**Inbox** = open/not-archived/not-trashed, mirroring Gmail's Inbox; **All mail** = everything bar Trash; Unread/Starred/Archived/Trash) with a Compose pill + Accounts, a Gmail rounded search bar, a **single main pane** (list XOR conversation, not a cramped split), **full-width single-line list rows** (checkbox · star · sender · subject — snippet · date that swaps to archive/trash/read **hover actions**), a **full-width conversation view** (back + toolbar, subject heading + star, message blocks with avatar/sender/date, Reply/Forward), and a **docked bottom-right composer** ("New message", From/To/Cc/Subject/body). Chrome is neutral; the exact Gmail-blue **`gmail` accent** (`packages/ui/tokens/colors.ts`, centred on #1a73e8) + amber (`secondary`) stars are a deliberate scoped comms theme (§37, mirroring the Trengo `inbox/*` exception) so it reads like Gmail, not the product purple — tokens only, no hardcoded hex. The page is **full-bleed** (no PageHeader, shell padding cancelled) so the client fills the viewport like Gmail. Keyboard shortcuts (j/k/e/s/u/#/r//c/Esc), debounced search, multi-select bulk actions (archive/read/trash), mark-read-on-open, rich-HTML reading pane (ADR 0041). tRPC `mail.accounts` + `mail.threads.list`; live via `useConversationStream`. |
