...
**Confidence threshold + matching.** Above-0.5-confidence extractions (ADR 0034 amendment — the matcher's unambiguous rule is the real safety, not the AI's self-confidence) resolve to a contact by **email → phone (E.164-normalised variants + unique 9-digit suffix) → name** (`matchContactByCandidate`, unit-tested). The name pass auto-links an unambiguous **first + last** name AND an unambiguous **single token / surname / whole-name-in-one-column** (so "spoke to Aanya" resolves), matching exactly ONE contact. Anything below threshold, name-ambiguous (two Jane Smiths), or unmatched lands in the "unassigned summaries" tray — but that tray is no longer a dead-end: the recurring **`slack/relink-unassigned`** job (§17.1) re-runs the matcher over every parked row for free and auto-links the unambiguous ones (draining the backlog + self-healing when a contact is created after the mention). Never auto-attach on ambiguity, never auto-create a contact (§3).
...
| `slack/relink-unassigned` | every 30 min | Re-run the contact matcher over every open `UnassignedSummary` and auto-link the unambiguous ones onto the contact's timeline as a `slack_summary` (ADR 0034 amendment). FREE — reuses the AI extraction already stored on the row + a deterministic email/phone text scan, never calls the AI again. Drains the parked backlog and self-heals (a mention parked before its contact existed links once the contact appears). Idempotent; ambiguous rows stay in the `/inbox/slack-mentions` tray (§3). |
...
| Open the dedicated email workspace | `/mail` (ADR 0021 Phase 4, `apps/web/app/(app)/mail/page.tsx` shell → `MailWorkspace.tsx` client). **Gmail-class layout**: an icon folder rail (Inbox/Unread/Starred/Archived/Trash) with a Compose pill + Accounts, a Gmail rounded search bar, a **single main pane** (list XOR conversation, not a cramped split), **full-width single-line list rows** (checkbox · star · sender · subject — snippet · date that swaps to archive/trash/read **hover actions**), a **full-width conversation view** (back + toolbar, subject heading + star, message blocks with avatar/sender/date, Reply/Forward), and a **docked bottom-right composer** ("New message", From/To/Cc/Subject/body). Chrome is neutral; the exact Gmail-blue **`gmail` accent** (`packages/ui/tokens/colors.ts`, centred on #1a73e8) + amber (`secondary`) stars are a deliberate scoped comms theme (§37, mirroring the Trengo `inbox/*` exception) so it reads like Gmail, not the product purple — tokens only, no hardcoded hex. The page is **full-bleed** (no PageHeader, shell padding cancelled) so the client fills the viewport like Gmail. Keyboard shortcuts (j/k/e/s/u/#/r//c/Esc), debounced search, multi-select bulk actions (archive/read/trash), mark-read-on-open, rich-HTML reading pane (ADR 0041). tRPC `mail.accounts` + `mail.threads.list`; live via `useConversationStream`. |
...
| Show the real Gmail sender name + custom labels in /mail | The sync captures the From header's **display name** (`parseFromName`, e.g. "Mohil Shah") and the thread's **custom Gmail labels** (`customLabelNames` — drops system + `CATEGORY_*`, mapped id→name via `listLabels`). Both live + backfill set `payload.senderName` on the Interaction and pass `senderName`/`labels` to `applyMailToConversation`, which keeps `Conversation.lastSenderName` (latest inbound sender) + `tags` (current Gmail labels). The list shows `lastSenderName` → matched contact → email (Gmail-style), not the matched CRM contact for everyone; label chips render on the list rows + conversation header. Without this the list wrongly fell back to one matched contact's name for every conversation. |
