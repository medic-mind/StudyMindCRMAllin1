...
| Manage lead-source API keys (Contact Form 7 sites) | Settings → Integrations → Lead webhook (`LeadIngestionPanel`, Manager+). tRPC `lead.sources.*` (list / create / update / rotate / archive / **delete**); schema `LeadSource` (sha256 key hash + last4, optional pinned brand). Raw key shown once. `delete` hard-removes the key (stops working immediately, irreversible); past leads are preserved (`Lead.sourceId` is `onDelete: SetNull`). `archive` is the reversible soft-disable. |
