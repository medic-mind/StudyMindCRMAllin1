...
**Google Voice ride-along (ADR 0031).** Google Voice has no call/SMS API, so we ingest its `voice-noreply@google.com` notification emails through this same Gmail sync: when the `google_voice.email_ingest_enabled` flag is on, `processMessage` hands those messages to the Google Voice handler instead of the normal email path. It logs a `call`/`message` Interaction (`source:'google_voice'`, voicemail + missed call flagged `needsManualReview`), reuses `resolveOrCreateContactForCall` to match/create the contact, and posts a best-effort team Slack alert. Voicemail audio streams to S3 like any attachment. This channel needs manual work by design (an agent types up the summary / checks the missed call). Live-call fidelity requires porting the number to a real telephony API (Twilio/Aircall).
