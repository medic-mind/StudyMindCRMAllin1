...
- **Never auto-mutate** (§2/§3): no auto-charge, auto-delete, or auto-send. AI
  suggests; a human confirms. **One operator-authorised exception:** *confident*
  duplicate contacts (shared email, or shared phone **and** matching name) are
  auto-merged (ADR 0047, §37) — everything ambiguous still waits for a human.
...
3. **No silent data mutation.** Never auto delete, never auto charge, never auto send. AI suggests, humans confirm. **Sole exception (operator-authorised, ADR 0047):** *confident* duplicate contacts — a shared email, or a shared phone **and** matching name — are auto-merged (hourly cron + a Manager+ "Run now"), audited like any merge. Ambiguous duplicates (phone-only with different names — a shared family landline, §41.1) are never auto-merged; they still surface on `/contacts/duplicates` for a human.
...
| `contacts/auto-merge-duplicates` | hourly | Auto-merge *confident* duplicate contacts — shared email, or shared phone + matching name — with no human step (ADR 0047, §37). Ambiguous ones (phone-only, different name) are left for the manual `/contacts/duplicates` page. Audited system merges; skips restricted-access conflicts. No-op with `CONTACTS_AUTO_MERGE=off`. |
...
- Do not auto charge cards, auto delete data, or auto send messages. AI suggests, humans confirm. (The one exception is auto-merging *confident* duplicate contacts — shared email, or phone + matching name — operator-authorised in ADR 0047; ambiguous ones still wait for a human.)
...
| Bulk-merge duplicate contacts | `/contacts` table → select 2+ → Merge (Manager+). tRPC `contact.bulkMerge` ({survivorId, loserIds}); first selected row is the survivor, the rest merge in via `mergeContacts` (`apps/web/lib/services/contact-merge.ts`) one at a time. **Auto-find duplicates** at `/contacts/duplicates` (Manager+): `contact.duplicates.find` clusters contacts sharing a normalised email or last-9-digit phone (union-find, `packages/core/src/contact/duplicates.ts`, unit-tested), one-click Merge per cluster. **Fully-automatic merging (ADR 0047):** the hourly `contacts/auto-merge-duplicates` cron (+ a Manager+ "Run auto-merge now" button → `contact.duplicates.autoMergeNow`) merges every *confident* duplicate with no human step — `planAutoMerges` keeps only members connected to the survivor by a **shared email** or a **shared phone AND matching name** (a phone-only match with a different name is a family landline, §41.1 — left for manual review). Merges run through the audited `mergeContacts` with a system actor and skip (never fail) restricted-access conflicts; shared logic in `apps/web/lib/services/auto-merge-duplicates.ts`, cron boundary `apps/web/app/api/inngest/_boundary/auto-merge-duplicates.ts` (disable with `CONTACTS_AUTO_MERGE=off`). Lead ingestion also dedupes format-insensitively (last-9 phone suffix + case-insensitive email, `buildPhoneMatch`) so re-enquiries never create a new dupe. |
...
- E.164 phone numbers are unique per `Contact` row but may legitimately repeat across a Family (shared landline). A phone-only match with a **different name** is never auto-merged — it surfaces as a manual merge candidate (§3/§37). A shared phone **with a matching name**, or a shared email, is a confident same-person signal and IS auto-merged (ADR 0047).
