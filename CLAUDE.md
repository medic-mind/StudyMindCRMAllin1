...
| Record + distribute a call summary (to customer vs internal Slack) | Both surfaces — board card modal (`apps/web/app/(app)/boards/[boardId]/CallSummarySection.tsx`, `card.callSummary.*`) and contact page (`apps/web/app/(app)/contacts/[id]/sections/CallSummarySection.tsx`, `contact.callSummary.*`) — split the send step into two clearly-labelled groups: **"To the customer"** (Trengo / Email + prefilled attachments: contact documents, uploaded invoices, template PDFs) and **"Internal — Slack & action points"** (post to a configurable Slack channel + optionally open a follow-up `Task` on the customer for a VA via `task.create`). Best-effort per channel (one failure never aborts the rest); records `call_summary` + `call_summary_sent` Interactions. Shared senders `apps/web/lib/board/call-summary-senders.ts`; pure Block Kit builder `packages/core/src/slack/blocks.ts`. |
...
**Schedule import.** A class's weekly topics can be typed, or **imported from a CSV / PDF / pasted text** (`webinar.syllabus.importPreview`): the text is extracted (PDFs via Node `zlib`, dependency-free), AI-structured into weekly topics (`webinar_schedule_import` mini-task) with a deterministic fallback, shown for human confirmation, then saved via `syllabus.set`. The uploaded PDF is still stored and attached to every reminder.

**Zoom integration (ADR 0035).** Optional. With a Zoom Server-to-Server OAuth app configured (`ZOOM_ACCOUNT_ID`/`ZOOM_CLIENT_ID`/`ZOOM_CLIENT_SECRET`), `webinar.class.generateZoomLink` (Manager+) creates a recurring meeting per class — **open to all** (join-before-host, no registration) with **cloud auto-recording** — and stores the join link + `zoomMeetingId`. The hourly `webinar/send-recordings` job emails each class its recording afterwards and, opt-in (`zoomTrashAfterSend`, default off), moves it to Zoom **Trash** (recoverable) after a successful send. All Zoom features fail closed when not configured. Client: `packages/integrations/zoom` (fetch-based, via `safeFetch`). Settings: Webinars → Settings.
