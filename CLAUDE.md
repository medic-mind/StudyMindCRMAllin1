...
**Channel-aware ingestion rules (ADR 0042).** The team's Slack layout encodes intent in the channel NAME, and the ingest honours it: a contact-linked mention from a channel whose name contains `complaint` (e.g. `#complaintcallsummaries`, `#b2bcomplaints`) **also auto-opens a Complaint** on the existing Complaints queue — open/medium, system-authored (`createdById` null); the labelled call format is parsed (`parseStructuredComplaint`, §37) into a clean title (`Complaint — <client name>`, else the first line) + a sectioned description, the customer is matched on the parsed CLIENT email/phone/name, category mapped from the AI category where it fits the presets, with the same timeline note + `complaint.created` audit as the human log flow. Idempotent on `Complaint.sourceKey` (`slack:<channelId>:<ts>`), applied on **every** ingestion path (live webhook, recurring pull, backfill, relink). **Only the thread's STARTING message opens a complaint** (`isThreadReplyMessage` — a message whose `thread_ts` differs from its own `ts` is a reply, 2026-07): the root is the complaint body; **thread replies never spawn their own Complaint** (they used to raise one complaint per reply, each mis-attributed to whoever the reply happened to name — the "every reply went to Neslie" bug). Replies are still captured as `slack_summary` mentions on their own timeline; they just don't open complaints. The reply gate lives in `maybeRaiseComplaintFromSlack` (`isThreadReply`), fed by all four ingestion paths. **Age gate = a go-live CUTOFF, not a rolling 7-day window** (`shouldAutoRaiseComplaint`, ADR 0042 amendment, mirroring the DD-issues cutoff §9): a message qualifies when it is still LIVE (≤7 days, the real-time fast path) **OR** its `occurredAt` is on/after `COMPLAINT_AUTO_RAISE_CUTOFF_DATE` (default `2026-07-01`). So EXISTING complaint-channel mentions since go-live open as Complaints (they used to be silently archived as mere mentions), while genuinely pre-CRM history still never floods the queue. Because the ingest idempotency guard short-circuits before the hook for already-archived messages, a one-off admin **reprocess** (`slack/backfill-complaints.requested` → `backfill-complaints.ts`, Settings → Integrations → Slack "Reprocess complaint mentions", Manager+) walks existing contact-linked `slack_summary` mentions ≥ cutoff and opens the complaints — idempotent on `sourceKey`, keyset-paged, self-rescheduling. The mention's own category chip is stamped `complaint` in those channels. Pure rules in `packages/integrations/slack/src/channel-rules.ts`; executor `complaints.ts` (best-effort — a complaint failure never blocks the archive).
...
| Auto-raise a Complaint from a Slack complaints channel (ADR 0042) | Pure rules + draft: `packages/integrations/slack/src/channel-rules.ts` (`isComplaintChannel` — name contains "complaint"; `shouldAutoRaiseComplaint` — contact-linked + [live ≤7 days OR `occurredAt` ≥ `COMPLAINT_AUTO_RAISE_CUTOFF_DATE`, default 2026-07-01, `resolveComplaintAutoRaiseCutoff`]; `buildComplaintDraft`); idempotent executor `complaints.ts` (`Complaint.sourceKey = slack:<channelId>:<ts>`); hooked into the live webhook (`jobs.ts`), pull/backfill (`backfill.ts`), relink (`relink.ts`) and reprocess (`backfill-complaints.ts`). **Only the thread's STARTING message opens a complaint** (`isThreadReplyMessage`, `maybeRaiseComplaintFromSlack`'s `isThreadReply` gate, 2026-07): a reply (its `thread_ts` ≠ its own `ts`) is the follow-up, not a new complaint — every path passes the flag (jobs.ts/backfill.ts via `message.thread_ts`, relink/reprocess via the archived envelope/`payload.threadTs`), so replies no longer flood the queue with one complaint each mis-attributed to whoever they named. **Structured import (ADR 0042 amendment):** the team's `#complaintcallsummaries` posts are a labelled call format (Client Name and Number / Guardian / Client Email / Hours / Amount Paid / Complaint bullets / Suggested Solution / Actions). `parseStructuredComplaint` (`complaint-parse.ts`, unit-tested against the real format) reads those fields, so the Complaint gets a CLEAN title (`Complaint — <client name>`, not the old first-line "Client Name and Number: …") and a sectioned description (narrative → suggested solution → actions → client/guardian/hours/amount), and the executor MATCHES the customer on the parsed CLIENT email → phone → name (then the guardian) via `matchContactByCandidate` — the authoritative details in the summary — rather than the generic mention link. Retroactive opener for existing mentions: `slack/backfill-complaints.requested` (`backfill-complaints.ts`, admin "Reprocess complaint mentions" button, Manager+). Complaints land on the normal `/complaints` queue. **Reverse direction (CRM → Slack):** a complaint LOGGED in the CRM (`complaint.create`) posts the same structured summary to the operator-routed complaint channel via the `complaint_call_summary` Slack topic (`buildComplaintSlack{Text,Blocks}` in `packages/core/src/slack/blocks.ts`; sender `apps/web/lib/complaints/slack-sender.ts`; best-effort, idempotent on the complaint id) — so logging a complaint here and typing one in Slack do the same thing. Slack-sourced complaints are created by the executor (not `complaint.create`), so there is no echo. |
