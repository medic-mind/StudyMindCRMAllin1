...
| `finance/dd-chase-tick` | hourly | Automated Direct Debit chasing (ADR 0045). Per armed case: auto-resolve when the GC mirror shows a fresh ACTIVE mandate (they signed back up — messages stop by themselves); else send the due chase on each enabled channel (email via system Gmail, SMS via Trengo under the case owner's token), walking the staff-authored `DdRecoveryTemplate` sequence so each step is more serious; when the sequence is exhausted, stop + flag for a human. Never sends without the staff-pasted re-signup link. Every send logged (`DdCaseMessage` + timeline note + audit). |
...
| Chase cancelled Direct Debits automatically (escalating emails/texts, per-person controls) | `/direct-debits/chasing` (ADR 0045). Manually add a customer (contact search + outstanding + re-signup link + email/text flags) or the nightly scan opens plan cases; nothing sends until staff paste the person's GoCardless/Stripe link. Engine `apps/web/app/api/inngest/_boundary/dd-chase.ts` (hourly); pure decisions `packages/core/src/finance/dd-chase.ts` (unit-tested); tRPC `finance.directDebit.cases.{chaseList,openManualChase,updateChase,markUpToDate,chaseMessages}` (reads all staff, writes Manager+); history `DdCaseMessage`; escalating copy authored at Settings → DD recovery templates (`{{setup_link}}` token). Stops on: fresh active mandate (auto), "Up to date ✓" (manual), pause, or sequence exhaustion (flags for a call). |
