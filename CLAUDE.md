...
| Import a whole weekly-classes timetable (PDF → cohort + classes + schedules) | `webinar.timetable.{importPreview,apply}` (`apps/web/app/api/trpc/routers/webinar.ts`); pure plan in `packages/core/src/webinar/timetable.ts`; AI prompt `packages/ai/src/prompts/webinar-timetable-import.ts`; UI `TimetableImport.tsx` on `/webinars/cohorts` (Manager+). Builds everything except Zoom links — staff fill those in (§47). |
