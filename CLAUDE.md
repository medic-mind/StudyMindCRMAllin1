...
| Sync Summer Camp bookings + view live camp roster/timetables (`camp.studymind.co.uk`) | `packages/integrations/summer-camp/` — **inbound** `webhook.ts` (HMAC verify) + `apply.ts` (match/create guardian `parent` + attendee `student` Contacts, `parent_of` link, `booking` Interaction on each timeline, audit `summer_camp.booking.*`) behind `jobs.ts` (`summer-camp/event.received`); receiver `apps/web/app/api/webhooks/summer-camp/route.ts` (secret `SUMMER_CAMP_WEBHOOK_SECRET`). **Outbound** read feeds via `client.ts` (`SUMMER_CAMP_API_URL`/`_API_KEY`, null when unset), tRPC `summerCamp.{status,camps,timetable}`, read-only UI `apps/web/app/(app)/camps/` (camps & fill grid + weekly timetables). Distinct from `booking.studymind.co.uk` (the tutoring platform). Contract: the camp repo's `CRM_INTEGRATION.md`. |
