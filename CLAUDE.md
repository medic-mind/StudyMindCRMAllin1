...
**Missed-calls workspace (`/calls`, all staff; action Sales Executive+).** A queue of inbound calls nobody answered (rang out OR voicemail), including unknown numbers. "Called back" is **derived** — a later outbound call to the same `rawDigits` (any attempt) auto-resolves the miss, so calling back from anywhere clears it with no manual step. A manual override (`actioned` / `dismissed` for spam) is the only stored state — `MissedCallReview`, keyed on the Aircall call id. Pure logic (dedupe + state) in `packages/core/src/calls/missed-calls.ts` (unit-tested); tRPC `calls.missed.{list,setReview,clearReview}`; UI `MissedCallsWorkspace.tsx` with click-to-call.
...
**Event-triggered backfill workers (ADR 0017).** Not recurring — fired once on first-connect (Gmail/Trengo) or by an admin button (Aircall/Slack/Summer Camp). `gmail/backfill.requested`, `aircall/backfill.requested`, `trengo/backfill.requested`, `slack/backfill.requested` each pull the last 90 days of history and write retroactive Interactions for matched Contacts. `summer-camp/backfill-bookings.requested` (admin button on `/camps`, CEO + Senior Manager) walks the camp app's keyset booking feed page-by-page (self-rescheduling on the cursor) to import ALL current bookings. `backfill/gocardless.requested` (button on `/finance/direct-debit`, CEO + Senior Manager — ADR 0038) walks the FULL GoCardless history (customers → mandates → subscriptions → payments, keyset cursor, self-rescheduling per page) into the complete provider mirror. Idempotent on the provider's native id; concurrency-capped (Slack 3 as it is AI-heavy, others 1–2). One summary audit row per job — never per imported message.
...
| Start a brand-new Trengo conversation with a contact | tRPC `interaction.trengo.startConversation` ({contactId, channel, body}, Sales Executive+) resolves the recipient (phone for whatsapp/sms, email for email), calls `outbound.startConversation` → `client.createConversation` (Trengo `/messages`), mirrors a `Conversation` head, echo-reconciled via the `interactionId` custom field. Recovered by `trengo/retry-pending-send`. UI: `StartTrengoConversation.tsx` on the contact's Trengo section. |
...
| Import Trengo history into an empty CRM (seed contacts) | "Import last 8 months" button on Settings → Integrations → Trengo (`TrengoImportButton.tsx`, CEO/Senior Manager, needs a connected Trengo token). tRPC `admin.backfill.trengoImport` ({windowDays≤366, createContacts}) → `startBackfill({provider:'trengo', createContacts})`. The worker (`packages/integrations/trengo/src/backfill.ts`) then **creates** a Contact for senders not already in the CRM — the explicit, operator-confirmed exception to §11's webhook default — keyed on phone/email (DB-deduped, re-run-safe), `kind:'parent'`, tagged `referralSource:'Trengo import'` so the batch is filterable. The 90-day auto-on-connect backfill keeps its `createContacts:false` matched-only behaviour. |
...
| Record + distribute a call summary (to customer vs internal Slack) | The **contact page** (`apps/web/app/(app)/contacts/[id]/sections/CallSummarySection.tsx`, `contact.callSummary.*`) is a **two-step flow**: **Step 1 — Send to the customer** over **WhatsApp / SMS / Email** (any combination; templates you click-to-prefill then edit, AI draft, and email attachments — contact documents, uploaded invoices, template PDFs); on send it advances to **Step 2 — internal note** (a *separate* note: what happened + next steps / VA instructions) saved to the CRM as a staff-only `note` Interaction (`contact.callSummary.logInternal`, never shown to the customer), optionally posted to a chosen Slack channel and/or opened as a follow-up `Task` for a VA. WhatsApp/SMS go via Trengo (continue the channel's thread, else start one to the contact's E.164 number — `resolveActiveTrengoConversation(db, contactId, channel)` + `startConversation`); Email replies on the latest Gmail thread. The **board card modal** (`apps/web/app/(app)/boards/[boardId]/CallSummarySection.tsx`, `card.callSummary.*`) uses the **same two-step flow** (compose + templates + AI draft → WhatsApp/SMS/Email → internal note); step 2 reuses `contact.callSummary.logInternal` via the card's backing contact. Best-effort per channel (one failure never aborts the rest); records `call_summary` + `call_summary_sent` (+ the internal `note`) Interactions. Channel senders `apps/web/lib/board/call-summary-senders.ts` (slack/trengo/**whatsapp**/**sms**/email); orchestrators `packages/core/src/{board,contact}/call-summary.ts`; pure Block Kit builder `packages/core/src/slack/blocks.ts`. |
| Archive Slack messages about a customer (survives 90-day retention) | ADR 0034. The Slack parser (`slack/event.received` + 90-day backfill) AI-classifies each watched-channel message and, on a confident email/phone match, writes a `slack_summary` Interaction on the Contact storing the **original message** (`messageText`, `senderName`, `channelId`, `permalink`) so the record outlives Slack's retention, plus an AI **category** (`billing·scheduling·feedback·complaint·academic·logistics·sales·general` — `slackSummarySchema`). Surfaced on the contact page Slack section (category chip + original text) via the `SlackMention` view-model. Unmatched/low-confidence references (incl. name-only) park in `UnassignedSummary` (now storing the original `messageText`/`senderName`) and surface in the **triage tray** at `/inbox/slack-mentions`: tRPC `slackSummary.unassigned.{count,list,assign,dismiss}` (Sales Executive+ to triage) — assigning writes the durable `slack_summary` Interaction on the picked customer + resolves the row; dismissing clears it. Never auto-create / auto-match (§12, §3). |
...
| Work the missed-calls queue / mirror all Aircall calls | `/calls` (all staff; action Sales Executive+). Inbound calls nobody answered (rang out OR voicemail), unknown numbers included. "Called back" is derived from a later outbound call to the same `payload.rawDigits`; manual override `MissedCallReview` (`calls.missed.{list,setReview,clearReview}`). Pure logic `packages/core/src/calls/missed-calls.ts`; UI `MissedCallsWorkspace.tsx`. Completeness: live webhooks + `aircall/sync-calls` cron + the historic backfill, all persisting matched **and** unmatched calls (`packages/integrations/aircall/src/{backfill,sync}.ts`). |
