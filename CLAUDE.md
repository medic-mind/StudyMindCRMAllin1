...
| Record + distribute a call summary (to customer vs internal Slack) | Both the **contact page** (`apps/web/app/(app)/contacts/[id]/sections/CallSummarySection.tsx`, `contact.callSummary.*`) and the **board card modal** (`apps/web/app/(app)/boards/[boardId]/CallSummarySection.tsx`, `card.callSummary.*`) render the shared **three-step wizard** `apps/web/components/contact/call-summary-wizard.tsx`: **Step 1 — Email?** (yes/no; compose from `CallSummaryTemplate` chips / AI draft / quick replies, optional subject for thread-less contacts, attach PDFs — **info packs from the document library (`InfoPackDocument`, §below)**, contact documents, uploaded invoices, template PDFs, device uploads); **Step 2 — Text or WhatsApp?** (yes/no; pick **WhatsApp** or **SMS**. WhatsApp surfaces the agent's approved **Trengo WhatsApp templates** (`contact.callSummary.waTemplates` → `listWhatsAppTemplates`, Trengo `GET /wa_templates`) with per-param inputs + live preview, sent as a real HSM via `sendWhatsAppTemplate` (Trengo `POST /wa_sessions` — works outside the 24h window, recovered by `trengo/retry-pending-send`); **no PDF picker on the template path** — the approved templates already carry the pack links, attaching again would duplicate. Free text / SMS keep the thread path — `resolveActiveTrengoConversation` + `sendMessage`/`startConversation`); everything sends in ONE audited fan-out (`send` with per-channel `channelBodies`, `emailSubject`, `whatsappTemplate`); **Step 3 — internal note** (staff-only `note` Interaction via `contact.callSummary.logInternal`, optional Slack post + VA follow-up `Task`). Email replies on the latest Gmail thread, else **starts a fresh thread** via `sendEmail`. Best-effort per channel (one failure never aborts the rest); records `call_summary` + `call_summary_sent` (+ the internal `note`) Interactions. Channel senders `apps/web/lib/board/call-summary-senders.ts`; orchestrators `packages/core/src/{board,contact}/call-summary.ts`; pure Block Kit builder `packages/core/src/slack/blocks.ts`. ADR 0039. |
