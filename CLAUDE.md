...
| Reply to a Trengo conversation from the CRM | tRPC `interaction.trengo.reply`; resolver `resolveActiveTrengoConversation` in `packages/integrations/trengo/src/conversations.ts`; reuses `outbound.ts` `sendMessage`. Send button on `components/contact/draft-reply-panel.tsx`. The comms-centre composer (`ConversationReply.tsx`) additionally offers a **WhatsApp approved-template mode** (Trengo `wa_templates` with inline `{{n}}` fill + live preview, sent via `interaction.trengo.startWhatsappTemplate` — defaults on when the 24h window has closed, because free text would be rejected). Thread rendering is Trengo-style: named senders per bubble (`payload.senderName` — customer for inbound, Trengo agent for outbound, CRM author resolved from `createdById`), day separators, label chips from `Conversation.tags`, and HTML email bodies rendered as readable text via `apps/web/lib/format/html-text.ts` (no raw-HTML injection, §44). Roadmap: ADR 0020. |
