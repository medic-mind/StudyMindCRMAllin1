...
| Export a list to CSV | `<CsvExportButton>` (`apps/web/components/ui/csv-export-button.tsx`) + `apps/web/lib/csv.ts` (RFC 4180 escape, UTF-8 BOM so Excel auto-detects). Mounted on `/contacts`, `/accounts`, and the InvoicesPanel header. List pages page through their tRPC procedure in 100-row chunks (cap 5000) so the export honours the current filter state. |
