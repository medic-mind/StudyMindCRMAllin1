...
> - **Named channels + Spam import + label dropdown + force-sync (fuller import pass)** — researched Trengo's API (`developers.trengo.com`): ticket `status ∈ {open, closed, spam}` and channels are `{id, name, type}`. Now: (a) the workspace's **named channels** ("business numbers": Support Manager, Tutor Manager, …) are mirrored (`TrengoChannel`, synced from `GET /channels` on import + team-sync), captured per conversation (`Conversation.trengoChannelId`/`trengoChannelName` via webhook/backfill/reconcile), surfaced as named **Channels** rail folders with counts + a thread-header channel chip so it's clear which line you're on; (b) **Trengo Spam is imported** — the `TICKET_MARKED_AS_SPAM`/`_UNMARKED_AS_SPAM` webhooks and the reconcile/backfill map `status:spam` into the Spam folder (previously spam was CRM-side only); (c) **labels** are a compact multi-select **dropdown** (OR/has-any, `list({tags})`) instead of a cluttered rail list; (d) the reconcile is faster (120/tick @ 10 min) and there's a **"Sync from Trengo"** button (`trengo/reconcile-now`) to force the recent open set to converge now. **Root-cause note:** the reason none of the earlier Trengo work appeared to "sync" in production is that every deploy had been failing to build since the webinar timetable feature shipped a reserved tRPC key (`apply`); fixed in `8c4b919`, so this is the first build that ships the reconcile + UI.
> - **Roadmap from the research** (not yet built, prioritised): Rules engine (label/assign/auto-reply/spam/forward/round-robin), auto-reply + business hours, agent online/away presence, the Reminder bell + Reminders folder, link/merge conversations, follow, broadcasts, CSAT, Live Dashboard reporting, on-demand Journey "macros", and AI reply-suggestions in the composer (the cheapest high-signal AI win, grounded on the existing knowledge base).
