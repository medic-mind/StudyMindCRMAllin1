...
**Phase 3c — SMS channel, contact-panel send, auto-clear.** `sendRecovery` now also sends **SMS via Trengo** (continues the contact's SMS thread, else starts one to their E.164 number; the Trengo outbound logs its own `message` Interaction) — the dialog has an Email/SMS toggle. The **Send message** action is also surfaced on the contact page Direct Debit panel (per plan with money still due, Manager+), reusing the same dialog (`gocardless.contactSummary` now returns `collectedMinor`/`expectedTotalMinor` for token rendering). And an open case **auto-clears to `recovered`** when its plan shortfall heals in `finance/flag-dd-defaulters` (system write, never reopens a written-off case; a manual Record-recovery with amount/ref is still available). WhatsApp HSM is deferred (free-text legal copy doesn't fit approved templates).
