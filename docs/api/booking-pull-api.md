...
> cheap. The CRM should be able to stay in sync by making a _handful_ of requests
...
### 2.6 Deletions & cancellations need to be _visible_ (tombstones)
...
**The CRM consumer is already built (ADR 0029).** `packages/integrations/booking/`
(`client.ts`, `student-sync.ts`, `jobs.ts`) implements exactly this contract and
is **dormant until `BOOKING_API_TOKEN` is set** — the sync jobs no-op without it,
so it is safe in production now. The moment the booking team ships the API and we
set the token, the four crons (§17.1 of CLAUDE.md) start mirroring:

- student → `Contact` (`kind = student`), keyed on `Contact.bookingContactId = uuid`;
  sets `bookingStatus`, `hoursBooked`, `hoursDelivered`, `bookingLastSyncAt`, plus a
  1:1 `ContactBookingProfile` (guardian, hours summary incl. premium + next expiry,
  credit balances).
- lesson → `BookingLesson` + a `booking` `Interaction` on the student contact;
  refreshes `Contact.lastLessonAt`.
- balance/credit ledgers → `BookingHoursTransaction` / `BookingCreditTransaction`
  (hours + credits carry **expiry** and a `_pence`/Stripe ref).
- `BusinessAccountStudent.syncFromBooking` calls the same client (per-student fetch).

Open follow-ups on our side: promote guardians to linked Contacts; recompute
precise per-lesson delivered hours + spend; feed lessons into reconciliation
(it still reads the legacy `Booking`/`BookingSession`); a Contact-page panel for
the ledgers. Env contract: `BOOKING_API_BASE_URL` (default `…/api/v1`) +
`BOOKING_API_TOKEN` (`.env.example`).
