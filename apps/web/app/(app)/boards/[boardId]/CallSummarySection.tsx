// Call summary inside the card detail modal — the same two-step workflow as the
// contact page so both surfaces behave identically:
//   Step 1 — compose (templates you can edit + AI draft) and send to the
//            customer over WhatsApp / SMS / Email (any combination; email
//            auto-attaches what you pick).
//   Step 2 — once the customer message is away, log a SEPARATE internal note
//            (next steps / VA instructions), saved to the CRM as internal-only
//            and optionally posted to a chosen Slack channel and/or opened as a
//            VA task.
// The summary persists as a call_summary Interaction on the card's backing
// contact; the internal note is logged via contact.callSummary.logInternal.
...
export function CallSummarySection({ cardId, canWrite }: Props) {
  ...
  >([])
  // Files uploaded straight from the agent's device (base64). Email only.
  ...
  const contactId = resolvedContactId ?? cardQuery.data?.contactId ?? ''
  ...
  const templatesQuery = trpc.callSummaryTemplate.pickList.useQuery(undefined, {
    enabled: canWrite,
  })
  const templates: DbTemplate[] = templatesQuery.data ?? []
  const activeTemplate = templates.find((t) => t.id === activeTemplateId) ?? null

  // Saved conversation templates ("Trengo templates" / quick replies).
  ...
  const slackChannelsQuery = trpc.slackChannel.pickList.useQuery(undefined, { enabled: canWrite })
  ...
  const vaQuery = trpc.task.assignableUsers.useQuery({}, { enabled: canWrite })
  const assignableUsers = vaQuery.data ?? []

  // Attachment sources — fetched up front (once we know the contact) so the
  // agent can pre-select a document/PDF regardless of channel.
  const documentsQuery = trpc.contact.documents.list.useQuery(
    { contactId },
    { enabled: canWrite && Boolean(contactId) },
  )
  ...
  async function submitStep1(alsoSend: boolean) {
    ...
    setBusy(true)
    try {
      const created = await add.mutateAsync({ cardId, body, outcome })
      setResolvedContactId(created?.contactId ?? null)

      if (alsoSend && (whatsapp || sms || email)) {
        // Attachments ride every chosen customer channel (WhatsApp/SMS via
        // Trengo media, email inline).
  ...
  async function submitStep2() {
    ...
    if (!contactId) {
      toast.error('Could not resolve the contact for this card.')
      return
    }
    setBusy(true)
    try {
    ...
    <section className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_10rem]">
        <Textarea
          ...
          placeholder={`What to send ${contactName} after the call…`}
        />
      ...
      <div className="rounded-lg border border-primary-200 bg-primary-50/40 p-3">
        ...
        <p className="mt-0.5 text-xs text-primary-900/70">
          {contactName} will receive this. Pick one or more channels.
        </p>
